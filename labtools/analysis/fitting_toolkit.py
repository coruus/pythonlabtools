...
This is loosely derived from the information in 'Numerical Recipes'
2nd Ed. by Press, Flannery, Teukolsky and Vetterling.

Implementation by Marcus H. Mendenhall,
Vanderbilt University Free Electron Laser Center, Nashville, TN, USA

Implemented around 3 December, 2002.

This implementation is in the public domain, although some of its
algorithms may be at least in part owned by the 'Numerical Recipes'
people.  Check with them before using this in commercial software.

To use it, subclass the fit class and, at minimum, define function(self,
params, x) as below
...
This function takes a Numeric array of parameters 'params', and a
Numeric array of independent variable values 'x'.  If the independent
variable is a scalar, the function will be passed a 1-d array.  If
the independent variable is a vector, the function will be passed
a 2-d array with each _row_ holding one component of the vector,
so x[0] would be x, x[1] would be y, etc.

Then, instantiate this class, give it initial parameter estimates,
set the autoderivative step size if numerical derivatives are being
used (the default value is 0.001*parameter values), add points, and
repeatedly call hessian_compute_fit until fitting has converged
(which, with Hessian fitting, it often won't!)
	...
	for i in range(5):
		...
		print(x.fitmat, x.fitvector, x.funcparams, x.funcvals[:x.pointcount],
              sqrt(x.chi2/x.pointcount))

To add more than one point at a time, do

	x.add_points(<array of independent variables>,
                 <array of dependent variables>,
                 x_transpose=<0 | 1>)

If the independent variable is a vector and the independent array
has the components of the vector in columns, so each column is one
vector, the default x_transpose=0 is correct.  If the components
are along rows, and each row is a vector, use x_transpose=1.

For a more robust fitter, use the Levenberg-Marquardt stepping as
follows e.g.  (this example shows one convergence checker):
...
For more advanced fitting (variable weight, analytic derivatives,
etc.) other methods can be overridden.  Note that except for the
function() override, all other function use raw access to the
internal variables of the fitter.  Since the data are stored in
arrays which are larger than the number of points, one must always
calculate based on self.xarray[:,self.pointcount] and
self.yarray[:self.pointcount] to return data of the right length.

If weighted fitting is desired, include the function weight_func(self)
in the subclass, \e e.g.
...
class fit:
  # Create the fitter, and give it a hint as to the size blocks to allocate
  # for data arrays.

  # If collect_garbage is True, the fitter will clean up with gc.collect() between passes, to reduce memory consumption.
  # Useful for huge fits. Override in subclass if huge-dataset capabilities
  # will be needed
  ...
  # Note in particular that the array returned is organized with all the
  # values for a point in each row.  The columns correspond to the
  # parameters.
  def derivs(self):
  ...
  #
  # This method should be overridden if anything fancy is being done with
  # weights (correlated fits, etc.)
  ...
  def setup_resampling(self):
    ...
    assert not hasattr(
        self, "saved_xarray"), "Don't even think of initializing the resampling more than once!"
    # these must be copies, not slices!
    self.saved_xarray = array(self.xarray[:, :self.pointcount])
  ...
  ##
  # Initialize the random number generator to be used in resampling.
  # Override this for non-default radnom generators.
  ...
  ##
  # return a list of \a count randoms, possibly by an efficient technique,
  # if the generator supports it.
  ...
  def resample(self):
    ...
    assert hasattr(
        self, "saved_xarray"), "resampling not set up yet.  Call setup_resampling() first."
    ranlist = Numeric.floor(
        self.get_random_list(self.pointcount) * self.pointcount).astype(numeric_int)

    # before we cone possibly huge arrays, clean up a bit to reduce huge
    ...
    # take columns since vectors lie this way
    self.xarray = Numeric.take(self.saved_xarray, ranlist, -1)
    self.yarray = Numeric.take(self.saved_yarray, ranlist)
    # it's a sequence if it has a __getitem__
    if hasattr(self.saved_explicit_weightlist, '__getitem__'):
      self.explicit_weightlist = Numeric.take(
          self.saved_explicit_weightlist, ranlist)
  ...
  def set_weights(self):
    ...
    self.scalar_weights = (
        type(self.weights)
        is type(1.0) or len(self.weights.shape) == 1)

  ##
  # Left-multiply a y-vector or function matrix by the weights.  If the
  # weights are special (sparse, etc.), override this.
  ...
  def compute_chi2(self):
    ...
    self.chi2 = float(
        dot(
            self.residuals, self.weights_multiply(
                self.residuals)))
  ...
  # clip the singular values to that factor less than the largest.  If the
  # variable does not exist, use 10^-10
  ...
  # If you directly call one of the specific fitters (hessian or svd), you
  # may never use this.
  ...
  ##
  # One choice of a schedule for adjusting the Levenberg-Marquardt
  # parameter.  Not the only one!
  def lm_lambda_recipe(self, old_chi2=None, new_chi2=None):
    ...
    if old_chi2 is None:
      ...
      if new_chi2 / old_chi2 < 1.1:
        # on a slightly bad fit (bobbling), don't penalize too badly
        self.lm_lambda *= 10.0
      else:
        # always jump to at least 1 on a bad fit
        self.lm_lambda = max(self.lm_lambda * 10.0, 1.0)
    else:
      self.lm_lambda *= 0.1
...
##
# Fit to a function which is a linear combination of basis functions, so
...
class linear_combination_fit(fit):
  ...
  def function(self, p, x):
    # first call from hessian_fit is meaningless, all coefficients zero, save
    # time
    if self.firstpass:
      return 0.0
  ...
  def derivs(self):
    # may get used more than once if weights are not constant, but no need to
    # recompute
    if self.firstpass:
      if hasattr(self, 'saved_derivs'):
        # if we have been reset to firstpass, but have really run before,
        # delete old stuff before creating new, since memory usage may be very
        # large
  ...
  ##
  # Given a vector of x values and y values, and optional weights, compute
  # the fit.
  def fit_data(self, xvector, yvector, weightlist=None):
    ...
    # one pass needed, no fancies, since the system is linear
    self.compute_fit()

##
# Fit a polynomial to data.  This is a special case of a linear
# combination fit, but code is optimized here
...
# save this for reuse, so find_peak can be called vey quickly
find_peak_fitter = polynomial_fit(2, pointhint=100)
...
def find_peak(data):
  ...
  topy = y[topchan]
  # find half-height points, starting a few points away to make it noise
  # resistant
  startx = topchan - 3
  while(startx > 0 and y[startx] >= 0.5 * topy):
    startx -= 1
  endx = topchan + 3
  while(endx < len(y) - 1 and y[endx] >= 0.5 * topy):
    endx += 1
  ...
  f.fit_data(
      x[startx: endx + 1],
      y[startx: endx + 1],
      xcenter=x[topchan])  # clears the fitter and does  fit
...
##
# Fit z0+a*exp( -(x-xmu)**2/(2*xsig**2) -(y-ymu)**2/(2.0*ysig**2)), an
# elliptical Gaussian
...
class gauss_fit_2d(fit):
  ...
  def function(self, p, r):
    ...
    xsigi = -1.0 / (2.0 * xsigma ** 2)
    ysigi = -1.0 / (2.0 * ysigma ** 2)
    return z0 + a * Numeric.exp(
        xsigi * (r[0] - xmu) ** 2 + ysigi * (r[1] - ymu) ** 2)
...
#
# This is a sample of a minimal fitting class which uses numerical
# derivatives, so all it has to declare is the function.
...
if __name__ == "__main__":
  ...
  class my_svd_bad_fit(fit):
    ...
    def function(self, p, x):
      # p[0] and p[1] are degenerate
      return p[0] * Numeric.exp(-p[1] + p[2] * x)
