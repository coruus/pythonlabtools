...
try:
  # prefer numpy over Numeric since it knows how to handle Numeric arrays,
  # too (maybe, but not reliably!)
...
class C2Function(object):
  ...
  # or \n
  # C2Function( \a first_C2, \a second_C2) sets the range to the
  ...
  def __init__(self, *args):
    ...
    # it should be two C2Functions, and we take the inner bounds of the two
    else:
      l, r = args
  ...
  # Return our name
  def __str__(self):
    return '<%s %s, Domain=[%g, %g]>' % (
        (self.ClassName, self.name,) + self.GetDomain())
  ...
  def __call__(self, arg):
    ...
    if isinstance(arg, C2Function):
      return C2ComposedFunction(self, arg)
    else:
      return self.value_with_derivatives(arg)[0]
  ...
  def find_root(self, lower_bracket, upper_bracket, start, value, trace=False):
    ...
    # can allow quite small steps, since we have exact derivatives!
    while abs(delta) > xtol:
      ...
      if disc < 0 or root <= lower_bracket or root >= upper_bracket or abs(delta) >= 0.5 * (upper_bracket - lower_bracket):
        # if we jump out of the bracket, or if the step is not smaller than
        # half the bracket, bisect
        root = 0.5 * (lower_bracket + upper_bracket)
        delta = upper_bracket - lower_bracket

      # get local curvature and value
      c, b, ypp = self.value_with_derivatives(root)
      c -= value
  ...
  def partial_integrals(self, recur_data, **args):
    ...
    # this is an initialization call, set everything up
    if type(recur_data[1]) is not tuple:
      ...
      if derivs == 0:
        ...
        raise C2Exception(
            "derivs passed to partial_integrals, must be 0, 1 or 2, got %d" %
            derivs)

      allow_rec = args.get('allow_recursion', True)
      # can only extrapolate after recursion
      extrapolate = args.get('extrapolate', True) and allow_rec
    ...
    (depth, funcgrid, old_integrals, absolute_error_tolerance,
     relative_error_tolerance, debug, extrapolate, derivs, eps_scale,
     extrap_coef, allow_rec) = recur_data
    ...
    for i in range(len(funcgrid) - 1):
      ...
      # check for underflow on step size, which prevents us from achieving
      # specified accuracy.
      if abs(dx) < abs(x1) * relative_error_tolerance:
        raise C2Exception(
            "Step size underflow in partial_integrals, depth = %d, x = %.4f" %
            (depth, x1))
      ...
      if derivs == 2:
        ...
        left = (((169 * ypp0 + 1024 * ypp1 - 41 * ypp2) * dx2 +
                 (2727 * yp0 - 5040 * yp1 + 423 * yp2)) * dx2 +
                (17007 * y0 + 24576 * y1 - 1263 * y2)) * (dx2 / 40320.0)
        right = (((169 * ypp2 + 1024 * ypp1 - 41 * ypp0) * dx2 -
                  (2727 * yp2 - 5040 * yp1 + 423 * yp0)) * dx2 +
                 (17007 * y2 + 24576 * y1 - 1263 * y0)) * (dx2 / 40320.0)
      elif derivs == 1:
        left = (
            (202 * y0 + 256 * y1 + 22 * y2) + dx *
            (13 * yp0 - 40 * yp1 - 3 * yp2)) * dx / 960.
        right = (
            (202 * y2 + 256 * y1 + 22 * y0) - dx *
            (13 * yp2 - 40 * yp1 - 3 * yp0)) * dx / 960.
      else:
        left = (5 * y0 + 8 * y1 - y2) * dx / 24.
        right = (5 * y2 + 8 * y1 - y0) * dx / 24.

      eps = abs(total - (left + right)) * eps_scale
      # the real error should be 2**order times smaller on this iteration, be
      # conservative
      if extrapolate:
        eps = eps * eps_scale
        # gain another factor of 2**order if extrapolating (typical), but be
        # conservative
  ...
  def GetSamplingGrid(self, xmin, xmax):
    ...
    if self.sampling_grid is None or xmin > self.sampling_grid[
            -1] or xmax < self.sampling_grid[0]:
      pass
      # dont really have any information on the funciton in this interval.
      return (xmin, xmax)
    else:
      ...
      # insert points  from source grid to destination into the middle of our
      # grid
      grid = [xmin] + list(sg[firstindex:lastindex]) + [xmax]

      # attempt to clear out points put too close together at the beginning, if
      # we have at least three points
      if len(grid) > 2:
        x0, x1, x2 = grid[:3]
        ftol = 10.0 * (1e-14 * (abs(x0) + abs(x1)) + 1e-300)
        if (x1 - x0) < ftol or (x1 - x0) < 0.1 * (x2 - x0):
          del grid[1]  # remove collision

      # attempt to clear out points put too close together at the end, if we
      # have at least three points
      if len(grid) > 2:
        x0, x1, x2 = grid[-3:]
        ftol = 10.0 * (1e-14 * (abs(x0) + abs(x2)) + 1e-300)
        if (x2 - x1) < ftol or (x2 - x1) < 0.1 * (x2 - x0):
          del grid[-2]  # remove collision
...
class C2ScaledRecip(C2Function):
  ...
  def value_with_derivatives(self, x):
    q = 1.0 / x
    return native(
        self.scale * q, -self.scale * q * q, 2 * self.scale * q * q * q)
...
class C2Quadratic(C2Function):
  ...
  def value_with_derivatives(self, x):
    dx = x - self.x0
    return native(
        self.a * dx * dx + self.b * dx + self.c, 2 * self.a * dx + self.b, 2 *
        self.a)
...
class C2Polynomial(C2Function):
  ...
  def __init__(self, coefs, x0=0.0):
    ...
    self.rcoefs = tuple(
        enumerate(self.coefs))[
        :: -1]  # record power index with coef
    self.xcenter = x0

  def value_with_derivatives(self, x):
    x -= self.xcenter
    xsum = 0.0
    for n, c in self.rcoefs:
      xsum = xsum * x + c
    xpsum = 0.0
    for n, c in self.rcoefs[:-1]:
      xpsum = xpsum * x + c * n
    xp2sum = 0.0
    for n, c in self.rcoefs[:-2]:
      xp2sum = xp2sum * x + c * n * (n - 1)
...
class C2ComposedFunction(C2Function):
  ...
  def __init__(self, outer, inner):
    if type(inner) is types.ClassType:
      inner = inner()  # instatiate unbound class
    if type(outer) is types.ClassType:
      outer = outer()  # instatiate unbound class
  ...
  def value_with_derivatives(self, x):
    return self.outer.compose(self.inner, x)
...
class C2BinaryFunction(C2Function):
  ...
  def __init__(self, left,  right):
    ...
    if isinstance(left, C2BinaryFunction):
      p1, p2 = '(', ')'
    else:
      p1, p2 = '', ''
    # put on parentheses to kepp hierachy obvious
    self.name = p1 + self.left.name + p2 + self.name + self.right.name
...
class C2Ratio(C2BinaryFunction):
  ...
  def value_with_derivatives(self, x):
    y0, yp0, ypp0 = self.left.value_with_derivatives(x)
    y1, yp1, ypp1 = self.right.value_with_derivatives(x)
    return y0 / y1, (yp0 * y1 - y0 * yp1) / (y1 * y1), (y1 * y1 * ypp0 + y0 *
                                                        (2 * yp1 * yp1 - y1 * ypp1) - 2 * y1 * yp0 * yp1) / (y1 * y1 * y1)
##
...
class C2Power(C2BinaryFunction):
  ...
  def value_with_derivatives(self, x):
    y0, yp0, ypp0 = self.left.value_with_derivatives(x)
    y1, yp1, ypp1 = self.right.value_with_derivatives(x)
    # all derivatives of right side are zero, save some time
    if isinstance(self.right, C2Constant):
      # this if a^(b-2), which appears often
      ab2 = _myfuncs.power(y0, (y1 - 2.0))
      ab1 = ab2 * y0  # this is a^(b-1)
      ab = ab1 * y0  # this is a^b
      ab1 *= yp0
      ab1 *= y1  # ab1 is now the derivative
      # ab2 is now the second derivative
      ab2 *= y1 * (y1 - 1) * yp0 * yp0 + y0 * y1 * ypp0
      return native(ab,  ab1, ab2)
    else:
      loga = _myfuncs.log(y0)
      # this if a^(b-2), which appears often
      ab2 = _myfuncs.exp(loga * (y1 - 2.0))
      ab1 = ab2 * y0  # this is a^(b-1)
      ab = ab1 * y0  # this is a^b
      yp = ab1 * (yp0 * y1 + y0 * yp1 * loga)
      ypp = ab2 * (y1 * (y1 - 1) * yp0 * yp0 + 2 * y0 * yp0 * yp1 *
                   (1.0 + y1 * loga) + y0 *
                   (y1 * ypp0 + y0 * (ypp1 + yp1 * yp1 * loga) * loga))
...
def _spline(x, y, yp1=None, ypn=None):
  if _has_linalg:
    ...
    u[1:-1] = dydx[1:]
    # this is an incomplete rendering of u... the rest requires recursion in
    # the loop
    u[1:-1] -= dydx[:-1]
...
def _spline_extension(x, y, y2, xmin=None, xmax=None):
  ...
  if xmin is not None:
    ...
    h1 = xmin - x[0]
    yextrap = y[
        0] + ((y[1] - y[0]) / h0 - h0 * (y2[0] + 2.0 * y2[1]) / 6.0) * h1 + y2[0] * h1 * h1 / 2.0
    yl.insert(0, (yextrap,))
    xl.insert(0, (xmin,))
    y2l.insert(0, (y2[0],))
  ...
  if xmax is not None:
    h0 = x[-1] - x[-2]
    h1 = xmax - x[-1]
    yextrap = y[-1] + ((y[-1] - y[-2]) / h0 + h0 * (2.0 * y2[-2] + y2[-1]
                                                    ) / 6.0) * h1 + y2[-1] * h1 * h1 / 2.0
    yl.append((yextrap,))
    xl.append((xmax,))
    y2l.append((y2[-1],))
...
def _splint(xa, ya, y2a, x, derivs=False):
  ...
  if not operator.isSequenceType(x):
    ...
    # this speeds things up, and makes sure numbers coming back from here are
    # native
    ...
    # if we got here, we are processing a list, and should do so more
    # efficiently
    ...
    npoints = len(x)
    khi = _numeric.clip(
        _numeric.searchsorted(xa, _numeric.asarray(x)),
        1, len(xa))
  ...
  y = a * ylo + b * yhi + ((a * a * a - a)
                           * y2lo + (b * b * b - b) * y2hi) * (h * h) / 6.0
  if derivs:
...
# these are functions to bypass the need for lambdas in the conversion
# functions.  This is compatible with pickling.
...
def _myexp(x):
  return _myfuncs.exp(x)


def _mylog(x):
  return _myfuncs.log(x)
...
class InterpolatingFunction(C2Function):
  ...
  def __init__(
          self, x, y, lowerSlope=None, upperSlope=None, XConversions=None,
          YConversions=None, cubic_spline=True):
    # just on general principle, right now this does nothing
    C2Function.__init__(self)
    self.SetDomain(min(x), max(x))
    self.Xraw = _numeric.array(x)  # keep a private copy
    self.xInverted = False

    if YConversions is not None:
      self.YConversions = YConversions  # inherit from class if not passed
    if self.YConversions is None:
      ...
      if lowerSlope is not None:
        lowerSlope *= self.fYinP(y[0])
      if upperSlope is not None:
        upperSlope *= self.fYinP(y[-1])

    if XConversions is not None:
      self.XConversions = XConversions  # inherit from class if not passed
    if self.XConversions is None:
      ...
      if lowerSlope is not None:
        lowerSlope /= self.fXinP(x[0])
      if upperSlope is not None:
        upperSlope /= self.fXinP(x[-1])

      # make sure our transformed X array is increasing
    ...
    if min(dx) < 0 or min(self.X) < self.X[0] or max(self.X) > self.X[-1]:
      raise C2Exception(
          "monotonicity error in X values for interpolating function: " +
  ...
  def value_with_derivatives(self, x):
    # skip this if this is a completely untransformed spline
    if self.xNonLin or self.yNonLin:
      y0, yp0, ypp0 = _splint(
          self.X, self.F, self.y2, self.fXin(x),
          derivs=True)
      y = self.fYout(y0)
      ...
      # the real derivative of the inverse transformed output
      yprime = gp * yp0 * fpi
      fpp = self.fYinPP(y)
      gpp = self.fXinPP(x)
      # also from Mathematica Dt[InverseFunction[f][g[y[x]]], {x,2}]
      yprimeprime = (
          gp * gp * ypp0 + yp0 * gpp - gp * gp * yp0 * yp0 * fpp * fpi * fpi) * fpi
  ...
  def SetLeftExtrapolation(self, bound):
    ...
    if xmin >= self.X[0]:
      raise C2Exception(
          "Attempting to extrapolate spline within its current range: bound = %g, bounds [%g, %g]"
          % ((bound,) + self.GetDomain()))

    self.X, self.F, self.y2 = _spline_extension(
        self.X, self.F, self.y2, xmin=xmin)
  ...
  def SetRightExtrapolation(self, bound):
    ...
    self.X, self.F, self.y2 = _spline_extension(
        self.X, self.F, self.y2, xmax=xmax)
  ...
  def YtoX(self):
    ...
    f = InterpolatingFunction(
        yv, yv, XConversions=self.XConversions, YConversions=self.YConversions)
  ...
  # and then re-splined.  If you want the full derivative information, use
  # C2ComposedFunction()

  def UnaryOperator(self, C2source):
    "return new InterpolatingFunction C2source(self)"
    y = [C2source(self.fYout(yy))
         for yy in self.F]  # get array of y values efficiently
    ...
    f = InterpolatingFunction(
        self.Xraw, y, lowerSlope=yp0, upperSlope=ypn,
  ...
  def BinaryOperator(self, rhs, c2binary):
    ...
    # get array of ( ( y, y', y''), ...)
    yv = [bf.value_with_derivatives(x) for x in self.Xraw]
    y = [yy[0] for yy in yv]  # get y values only

    f = InterpolatingFunction(
        self.Xraw, y, lowerSlope=yv[0][1],
        upperSlope=yv[-1][1],
        XConversions=self.XConversions, YConversions=self.YConversions)
    f.name = bf.name
    return f
...
class AccumulatedHistogram(InterpolatingFunction):
  ...
  def __init__(
          self, binedges, binheights, normalize=False, inverse_function=False,
          drop_zeros=True, **args):
    be = _numeric.array(binedges, _numeric_float)
    bh = _numeric.array(binheights, _numeric_float)

    # invese functions cannot have any zero bins or they have vertical sections
    if drop_zeros or inverse_function:

      nz = _numeric.not_equal(bh, 0)  # mask of non-empty bins, or lower edges
      if not inverse_function:
        # always preserve end bins to keep X range, but don't dare do it for
        # inverses
        nz[0] = nz[-1] = 1
      bh = _numeric.compress(nz, bh)
      be = _numeric.compress(_numeric.concatenate((nz, (nz[-1],))), be)

    cum = _numeric.concatenate(
        ((0,), _numeric.cumsum((be[1:] - be[:-1]) * bh)))

    if be[1] < be[0]:  # fix backwards bins, if needed.
      be = be[::-1]
      cum = cum[::-1]
      # the dx values were all negative if the bins were backwards, so fix the
      # sums
      cum *= -1
...
class InverseIntegratedDensity(InterpolatingFunction):
  ...
  def __init__(self, bincenters, binheights):
    ...
    # must be a data table, create interpolator
    if not isinstance(binheights, C2Function):
      np = len(binheights)
      ...
      # // create a temporary InterpolatingFunction to integrate
      temp = self.IntermediateInterpolator(be, bh)
    else:
      temp = binheights

    # integrate from first to last bin in original order, leaving results in integral
    # ask for relative error of 1e-6 on each bin, with absolute error set to 0
    # (since we don't know the data scale).
    ...
    InterpolatingFunction.__init__(
        self, integral, bincenters, lowerSlope=1.0 /
        (scale * temp(bincenters[0])),
        upperSlope=1.0 / (scale * temp(bincenters[-1])))  # use integral as x axis in inverse function
...
class C2InverseFunction(C2Function):
  ...
  def __init__(self, sourceFunction):
    ...
    # compute our domain assuming the function is monotonic so its values on
    # its domain boundaries are our domain
    ly = sourceFunction(l)
    ry = sourceFunction(r)
    if ly > ry:
      ly, ry = ry, ly
  ...
  def value_with_derivatives(self, x):
    ...
    # always start looking near previous solution, unless value is reset
    # explicitly
    self.start_hint = y
...
class C2ConnectorFunction(C2Function):
  ...
  def __init__(self, f1, f2, x0, x2, auto_center=False, y1=0.0):
    ...
    # get left wall values from conventional computation
    y0, yp0, ypp0 = f1.value_with_derivatives(x0)
    # get right wall values from conventional computation
    y2, yp2, ypp2 = f2.value_with_derivatives(x2)

    # scale derivs to put function on [-1,1] since mma  solution is done this
    # way
    yp0 *= fdx
    yp2 *= fdx
    ypp0 *= fdx * fdx
    ...
    if auto_center:
      y1 = ff0  # forces ff to be 0 if we are auto-centering
  ...
  def value_with_derivatives(self, x):
    ...
    # exactly vanish all bits at both ends
    q1 = (x - x0) * (x - x2) * fhinv * fhinv
    q2 = dx * q1
    ...
    q4 = 5 * q1 + 4
    yprime = (
        self.fa + 2 * self.fb * dx + self.fd * q2 + r2 * q3 + self.ff * q1 * q2 +
        q1 * q4 * r3) * fhinv
    yprime2 = 2 * (self.fb + self.fd * q3 + 3 * dx * r2 + self.ff * q1 * q4 + r3 *
                   (2 * dx * (5 * q1 + 2))) * fhinv * fhinv
...
class C2LHopitalRatio(C2Ratio):
  ...
  # eps0 sets the range around a denominator zero which triggers L'Hopital's
  # rule.  If abs(dx) < eps0*x, the rule is used
  epsx1 = 1e-4
  epsx2 = 1e-4
  eps0 = 1e-5
  eps1 = 1e-14
  ...
  def dxsolve(self, x, y, yp, ypp):
    ...
    if disc >= 0:
      ...
      if q * q > abs(a * c):
        # since x1=q/a, x2=c/q, x1/x2=q^2/ac, this picks smaller step first
        delta = c / q
      else:
        delta = q / a
    else:
      raise C2Exception(
          "Thought a root should be near x= %g, didn't find one" % x)
  ...
  def value_with_derivatives(self, x):
    ...
    cache = self.cache
    # can't get it out of cache, must compute something
    if cache is None or x < cache[0] or x > cache[1]:
      y0, yp0, ypp0 = self.left.value_with_derivatives(x)
      y1, yp1, ypp1 = self.right.value_with_derivatives(x)

      # paranoid check for relative proximity to a simple denominator zero
      if (abs(y1) < self.eps0 * abs(yp1) * (abs(x) + self.dblmin)):
        ...
        # get left wall values from conventional computation
        y0, yp0, ypp0 = C2Ratio.value_with_derivatives(self, x0)
        y2, yp2, ypp2 = C2Ratio.value_with_derivatives(
            self, x2)  # get right wall values
      ...
      # not close to a zero of the denominator... compute conventional answer
      # for ratio
      else:
        return y0 / y1, (yp0 * y1 - y0 * yp1) / (y1 * y1), (y1 * y1 * ypp0 + y0 *
                                                            (2 * yp1 * yp1 - y1 * ypp1) - 2 * y1 * yp0 * yp1) / (y1 * y1 * y1)
...
if __name__ == "__main__":
  ...
  if 0:
    ...
    for lv in (0.1, 1.0, 2.0, 5.0, 10.0):
      ...
      v1 = C2recip.partial_integrals(
          _numeric.array((1.0, math.sqrt(b),
                          b)),
          absolute_error_tolerance=1e-6, extrapolate=1, debug=0, derivs=0)
      n1 = C2recip.total_func_evals

      v2 = C2recip.partial_integrals(
          _numeric.array((1.0, math.sqrt(b),
                          b)),
          absolute_error_tolerance=1e-12, extrapolate=1, debug=0, derivs=1)
      n2 = C2recip.total_func_evals

      v3 = C2recip.partial_integrals(
          _numeric.array((1.0, math.sqrt(b),
                          b)),
  ...
  if 0:
    ...
    pf = LinLogInverseIntegratedDensity(
        energies[:: -1],
  ...
  if 1:
    ...
    for fn, fn0 in(
        ...
        (C2LHopitalRatio(C2sin, C2Linear(y0=-math.pi) / C2Linear()),
