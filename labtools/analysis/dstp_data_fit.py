...
# create a data cache with neatly nameable elements
...
# note that for DSTP devices, the <internal name on device> much match the
# DSTP URL given in labVIEW (not case sensitive)
data.define_field(
    "XY", s, ("xyvals", [array.array('d', []), array.array('d', [])]), writable=0)
data.define_field(
    "fitparms", s, ("parms", [0, array.array('d'),
                              0.0, array.array('d')]),
    writable=1)
...
try:
  ...
  def run_fit(string):
    ...
    try:
      # note that arrays get passed flattened, and with the original
      # dimensioning info in a tuple with the array.  I will fix this later.
      (xl, x), (yl, y) = data.XY

      # print x,y
      # make sure each time we send data, this is different, so LabVIEW notices
      # the change
      loopcount += 1
      ...
      data.fitparms = [
          loopcount, y, math.sqrt(Numeric.sum((y - 1) * (y - 1))),
          y - 1]  # send computed results back to LabVIEW

    except:  # clean up on errors
      # dump what went wrong, then close the server so we can edit the program
      traceback.print_exc()
      s.close()

  # tell the server that any time new data appears in 'xyvals' to call
  # 'run_fit'
