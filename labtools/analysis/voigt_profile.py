...
# so that cache thrashing is avoided if one is computing for many peaks
# with widely varying parameters
...
class Voigt_calculator:
  ...
  def __init__(self):
    ##
    # This is a cache which is used to store the most recently computed data
    # grids, so similar computations can be done very quickly.
    # k_points, xfullwidth, kvals, xvals, k2, cosarray, x2
    self.saved_params = (0, 0, None, None, None, None)
  ...
  def voigt_with_derivs(
    ...
    if k_points is None:
      ...
      # the Numerical Recipes quadratic trick
      q = -0.5 * (
          alpha + math.sqrt(alpha * alpha + 4.0 * kexptail * (0.5 * sigma ** 2)))
      k1 = -kexptail / q
      kp = k1 * xfullwidth / math.pi
      # next power of two up
      k_points = 2 ** int(1 + math.floor(math.log(kp) / math.log(2.0)))

    if self.saved_params[:2] != (k_points, xfullwidth):
      kmax = math.pi * k_points / (xfullwidth)
      kvals = Numeric.array(
          range(k_points + 1), _numeric_float) * (kmax / k_points)
      dx = xfullwidth / (2.0 * k_points)
      xvals = Numeric.array(
          range(2 * k_points), _numeric_float) * (xfullwidth / float(k_points)) - xfullwidth
      k2 = kvals * kvals
      x2 = xvals * xvals
    ...
    yvals = Numeric.exp(
        Numeric.clip((-0.5 * sigma * sigma) * k2 - alpha * kvals, -100, 0))
    ...
    # transform of df/d(sigma) = d(transform of f)/d(sigma) =
    # -k*k*sigma*transform
    yvals *= kvals
...
if __name__ == '__main__':
  ...
  stylecolors = [
      GracePlot.green, GracePlot.blue, GracePlot.red, GracePlot.orange,
      GracePlot.magenta, GracePlot.black]
  s1, s2, s3, s4, s5, s6 = [GracePlot.Symbol(
      symbol=GracePlot.circle, fillcolor=sc,
      size=0.5, linestyle=GracePlot.none)
      for sc in stylecolors]
  l1, l2, l3, l4, l5, l6 = linestyles = [
      GracePlot.Line(type=GracePlot.solid, color=sc, linewidth=2.0)
      for sc in stylecolors]
  ...
  if 0:
    ...
    datasets.append(
        GracePlot.Data(x=xvals, y=vgt * scale,  type='xy', line=l1))
    datasets.append(
        GracePlot.Data(x=xvals, y=dsig * sigma0,  type='xy', line=l2))
    datasets.append(
        GracePlot.Data(
            x=xvals, y=dalpha * alpha0, type='xy', line=l3))
    datasets.append(
        GracePlot.Data(
            x=xvals, y=5000.0 * alpha0 * (vgta1 - vgta2),
            type='xy', symbol=s3, line=noline))
    datasets.append(
        GracePlot.Data(
            x=xvals, y=5000.0 * sigma0 * (vgts1 - vgts2),
            type='xy', symbol=s2, line=noline))
  else:
    ...
    class lorentz(C2Functions.C2Function):
      ...
      def value_with_derivatives(self, x):
        ...
        a = 1.0 / (1.0 + x * x)
        return a * scale, -2 * x * a * a * scale / self.width, (
            6 * x * x - 2) * a * a * a * scale / self.width ** 2
    ...
    for pidx, (xfw, sigma0) in enumerate(((500., 1.),
                                          (500., 50.),
                                          (2000., 50.))):
      t0 = time.time()
      for i in range(100):
        xvals1, vgta1, dsig, dalpha = voigt_with_derivs(
            sigma0, alpha0, kexptail=25, xfullwidth=xfw)
      t1 = time.time()
      ...
      gg.set_width(sigma0)
      for x in xvals1:
        # need to cast to float if numpy is used to avod horrible numpy-objects
        # as the float
        ll.set_center(float(x))
        convlist.append(prod.integral(-10.0 * sigma0, 10.0 * sigma0,
                                      relative_error_tolerance=1e-13, absolute_error_tolerance=1e-16))
      ...
      datasets.append(
          GracePlot.Data(
              x=xvals1, y=vgta1, type='xy', line=linestyles[2 * pidx],
              legend=r"Function: \xs\f{} = %.0f, \xD\f{} = %.0f" %
              (sigma0, 2 * xfw)))
      datasets.append(
          GracePlot.Data(
              x=xvals1, y=diff / vgta1 * 1e3, type='xy',
              line=linestyles[2 * pidx + 1],
              legend=r"10\S3\N \x\#{b4}\f{} Relative Error: \xs\f{} = %.0f, \xD\f{} = %.0f"
              % (sigma0, 2 * xfw)))
  ...
  g.yaxis(label=GracePlot.Label('Amplitude'), scale='logarithmic',
          ymin=1e-6, ymax=1, tick=GracePlot.Tick(major=10, minorticks=9))

  graceSession.send_commands(
      'hardcopy device "PDF"', 'print to "voigt_errors.pdf"')
