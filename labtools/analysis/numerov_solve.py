...
def single_well_numerov_match(V0, dx, parity=1, overlapsize=5):
  ...
  right_turn = numpy.searchsorted(
      V0[vminchan:],
      0.0) + vminchan  # position of right-hand turning point

  # integrate from left end to right-hand turning point, at which point
  # numerov method becomes unstable
  leftpsi = bare_numerov(V0[:right_turn], dx)
  # iterate backwards on right part to turning point, and flatten to normal
  # array
  rightpsi = numpy.array(
      bare_numerov(
          V0[right_turn - overlapsize:][:: -1],
          dx)[:: -1])

  # remember that since numpy handles slices without copying, leftend and
  # rightend also scale here
  slopeweight = numpy.array(
      range(overlapsize),
      numpy.float) - overlapsize / 2.0
  leftend = leftpsi[-overlapsize:]
  rightend = rightpsi[:overlapsize]

  # 'least-squares' scale estimate
  scale = abs(numpy.sum(leftend * rightend) / numpy.sum(leftend ** 2))
  # note that since leftend and rightend are numpy slice references,
  # modifying psi also changes them!
  ...
  # use simpson's rule
  integral = psi2[
      0] + psi2[-1] + 4.0 * numpy.sum(psi2[1:-1:2]) + 2.0 * numpy.sum(psi2[2:-2:2])
...
def single_well_numerov_solve(V0, dx, parity, emin, emax, overlapsize=5):
  ...
  e = zbrent(
      lambda
      x:
      single_well_numerov_match(
          V0 - x, dx, parity, overlapsize=overlapsize)[1],
      emin, emax, 1e-15, trace=0)
  psi, slope_error = single_well_numerov_match(V0 - e, dx, parity, overlapsize)
  return e, psi


def find_spectrum(
        V0, dx, levels, ground_state_estimate, state_separation_estimate=None,
        trace=0, subdivide=4):
  results = []
  if state_separation_estimate is None:
    state_separation_estimate = ground_state_estimate
  # prime the bookkeeping to guess where next state lies and make it point
  # to ground_state_estimate
  e0, e1 = ground_state_estimate - 2 * \
  ...
  for i in range(levels):
    ...
    try:
      ...
      psi, slope = single_well_numerov_match(
          V0 - firstguess, dx, parity)  # initial slope
      for s in range(1, 10 * subdivide):
        ...
        firstguess = e_test
        slope = test_slope
      e_n, psi_n = single_well_numerov_solve(
          V0, dx, parity, e1 + (s - 1) * delta_e, e_test)  # must bracket root
...
if __name__ == '__main__':
  ...
  xar = (numpy.array(range(potlen),
                     numpy.float) - (potlen - 1) / 2.0) * (xmax * 2.0 /
                                                           (potlen - 1))
  testpot = xar ** 2
