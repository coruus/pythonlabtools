...
class spitfire:
  ...
  spitfire_abcd = (optics.mirror(1000.0 * mm) * optics.space(19.0 * inch) *
                   optics.mirror(500.0 * mm) * optics.space(9.5 * inch) *
                   spitfire_thermal * optics.space(9.5 * inch) * optics.mirror(
                       500.0 * mm) * optics.space(19.0 * inch) * optics.mirror(
                       1000.0 * mm) * optics.space(19.0 * inch) * optics.mirror(
                       500.0 * mm) * optics.space(9.5 * inch) * spitfire_thermal
                   * optics.space(9.5 * inch) * optics.mirror(500.0 * mm) *
                   optics.space(19.0 * inch))
...
def draw_layout(graph, optics):
  ...
  colorline.lineStyle = graphite.LineStyle(
      width=1, color=graphite.orange, kind=graphite.SOLID)
...
def draw_trace(graph, optics_trace):
  ...
  colorline.lineStyle = graphite.LineStyle(
      width=1, color=color, kind=graphite.SOLID)
  colorline.symbol = None
...
def draw_everything(optics, trace, xrange, yrange, three_d=None):
  ...
  g.axes[graphite.Y].tickMarks[0].labelStyle = graphite.TextStyle(
      hjust=graphite.RIGHT, vjust=graphite.CENTER, font=graphite.Font(
          10, 0, 0, 0, None),
  ...
  g.axes[graphite.X].tickMarks[0].labelStyle = graphite.TextStyle(
      hjust=graphite.CENTER, vjust=graphite.TOP, font=graphite.Font(
          10, 0, 0, 0, None),
      color=graphite.Color(0.00, 0.00, 0.00))
...
class blue_through_ylf(composite_optic):
  ...
  UVM1, UVM2, UVM3, UVM4, UVM5, UVM6, UVM7, UVM8, UVM9, UVM10, UVM11, UVM12 = range(
      12)
  PS1u, PS1d = 'ps1u', 'ps1d'
  ...
  def __init__(self, **extras):
    ...
    optics[
        my.PS1u] = mir(
        "Blue Peri 1 upper", (8 * inch, entrance_y, 37 * inch))
    ...
    optics[
        my.YLF_SPLIT] = mir(
        "YLF exit splitter", (48 * inch, y0, 47.8 * inch))

    optics[my.UVL1] = lens(
        "UVL1", f=+0.75).place_between(optics[my.UVM3], optics[my.UVM4], 4.0 * inch)
    optics[
        my.UVL2] = lens(
        "UVL2", f=+1.00).place_between(
        optics[my.UVL1],
        optics[my.UVM4],
        83 * inch - 0.0 * inch)

    optics[my.DEMAG1] = lens(
        "UVL3", f=+0.75).place_between(optics[my.UVM9], optics[my.UVM10], 23.0 * inch)
    optics[my.DEMAG2] = lens(
        "UVL4", f=+1.00).place_between(optics[my.DEMAG1], optics[my.UVM10], 1.7 + 5.0 * inch)

    optics[my.SERR] = null_optic("Serr. Aperture", width=1.0 * inch,
                                 thickness=0.1 * inch).place_between(optics[my.DEMAG1],
                                                                     optics[my.DEMAG2],
                                                                     3 * inch)
    optics[my.FILTER] = null_optic("Filter Aperture", width=1.0 * inch,
                                   thickness=0.1 * inch).place_between(optics[my.DEMAG1],
                                                                       optics[my.DEMAG2],
                                                                       0.75)

    looks = [
        my.UVM1, my.UVM2, my.PS1u, my.PS1d, my.UVM3, my.UVM4, my.UVM5, my.UVM6,
        my.UVM7, my.UVM8, my.UVM9, my.UVM10, my.UVM11, my.UVM12, my.YLF_SPLIT]
    # align all mirrors
    for i in range(1, len(looks) - 1):
      optics[
          looks[i]].set_direction(
          optics[looks[i - 1]],
          optics[looks[i + 1]])

    order = [
        my.UVM1, my.UVM2, my.PS1u, my.PS1d, my.UVM3, my.UVL1, my.UVL2, my.UVM4,
        my.UVM5, my.UVM6, my.UVM7, my.UVM8, my.UVM9, my.DEMAG1, my.SERR, my.
        FILTER, my.DEMAG2, my.UVM10, my.UVM11, my.UVM12, my.YLF_SPLIT]

    # specify center and reference as (0,0,0) to make our coordinates absolute
    composite_optic.init(
        self, "blue line", optics, order, (0, 0, 0),
        (0, 0, 0),
        0, extras=extras)
...
class ir_line(composite_optic):
  ...
  def __init__(self, **extras):
    ...
    optics[
        my.BP4] = PL_brewster_filter(
        "BP4", (33 * inch, entrance_y, 27 * inch),
        reversed=1)
    ...
    optics[
        my.EXIT] = sys_exit = null_optic(
        "exit", (10.0 * inch, exit_y, 142.0 * inch))

    optics[my.L1] = lens(
        "L1", f=+0.400).place_between(optics[my.M2], optics[my.TrM1], 12.0 * inch)
    optics[my.L2] = lens(
        "L2", f=+0.800).place_between(optics[my.L1], optics[my.TrM1], 1.30)
    optics[my.L3] = lens(
        "L3", f=+0.750).place_between(optics[my.M6], optics[my.M7], 10 * inch)
    optics[my.L4] = lens(
        "L4", f=+1.500).place_between(optics[my.L3], optics[my.M7], 2.25)
    optics[my.L5] = lens(
        "L5", f=+0.750).place_between(optics[my.M10], optics[my.M11], 5 * inch)
    optics[my.L6] = lens(
        "L6", f=+1.500).place_between(optics[my.L5], optics[my.M11], 95.5 * inch)
    optics[my.L7] = lens(
        "L7", f=+0.750).place_between(optics[my.M15], sys_exit, 4.5 * inch)
    optics[
        my.L8] = lens(
        "L8", f=+2.500, width=6 * inch, thickness=0.25 * inch).place_between(
        optics[my.M15],
        sys_exit, -1 * inch).tilt_off_axis(
        (0, 45, 0))
    ...
    optics[
        my.QR1] = halfwave_plate(
        "QR1", width=1 * inch, thickness=0.5 * inch).place_between(
        optics[my.G9a],
        optics[my.G9b],
    ...
    optics[
        my.QR2] = halfwave_plate(
        "QR2", width=1 * inch, thickness=0.5 * inch).place_between(
        optics[my.G25a],
        optics[my.G25b],
        8 * inch).rotate_axis(22.5)

    optics[
        my.BP2] = PL_brewster_filter(
        "BP2", transmit=1).place_between(
        optics[my.G25b],
        optics[my.M9],
        13 * inch).rotate_axis(90)
    optics[
        my.QR3] = halfwave_plate(
        "QR3", width=1 * inch, thickness=0.5 * inch).place_between(
        optics[my.G25b],
        optics[my.M9],
    ...
    order = [
        my.M1, my.M2, my.SERR1, my.L1, my.SF1, my.L2, my.TrM1, my.TrM2, my.
        TrM3, my.TrM4, my.M3, my.M4, my.G9a, my.QR1, my.G9b, my.M5, my.M6, my.
        FR12, my.L3, my.VSF1, my.L4, my.M7, my.M8, my.G25a, my.QR2, my.G25b,
        my.BP2, my.QR3, my.FR25, my.BP3, my.M9, my.M10, my.L5, my.L6, my.G50,
        my.M11, my.BP4, my.M12, my.M13, my.M14, my.M15, my.L7, my.L8]

    # specify center and reference as (0,0,0) to make our coordinates absolute
    composite_optic.init(
        self, "IR line", optics, order, (0, 0, 0),
        (0, 0, 0),
        0, extras=extras)
...
class ir_compressor(composite_optic):
  ...
  def __init__(
          self, theta1=61.486, clen=1.02, lambda0=1.054e-6, center=(0, 0, 0),
          angle=0, **extras):
  ...
  def init(self, theta1, clen, lambda0, center, angle, extras):
    ...
    optics[
        my.INPUT] = m1 = reflector(
        "input mirror", center=(0, input_height, 0),
        width=8 * inch, angle=-45)

    optics[
        my.G1] = g1 = grating(
        "g1", angle=theta1 + 90, center=(52 * inch, center_height, 0),
    ...
    optics[
        my.G2] = g2 = grating(
        "g2", angle=theta1 - 90, center=mybeam.x0 + (0, 0, 1e-8),
        pitch=1740e3, order=1, width=16 * inch, thickness=2.0 * inch)

    optics[
        my.VR1] = reflector(
        "retro", center=(40 * inch, center_height, g2.center[2]),
        angle=90.0, width=8 * inch, thickness=1.0 * inch)

    out = optics[
        my.OUTPUT] = reflector(
        "output mirror", center=(8 * inch, output_height, 0.0),
        width=8.0 * inch, thickness=1.0 * inch)
    focus = optics[
        my.FOCUS] = lens(
        "output focus", center=out.center + (35 * inch, 0, 7 * inch),
        angle=90 - math.atan(0.2) / deg, f=2.0, width=8.0 * inch, height=8.0 *
        inch, thickness=1.0 * inch).tilt_off_axis(0)
    out.set_direction(g1.center - (0, center_height - output_height, 0), focus)

    comp_order = (
        my.INPUT, my.G1, my.G2, my.VR1, my.G2, my.G1, my.OUTPUT, my.FOCUS)

    composite_optic.init(
        self, "ir compressor", optics, comp_order, None, center=center,
        angle=0, extras=extras)
...
class blue_compressor(composite_optic):
  ...
  def setup_retro_standard(
          self, beam, vertex_distance, retro_beam_offset, retro_err, y_center):
    ...
    ir1 = reflector(
        my.IR1, angle=(beam_angle - 45 - retro_err / 2),
        center=(vx, y_center, vz),
        justify="left", width=0.05)
    ir2 = reflector(
        my.IR2, angle=(beam_angle + 45 + retro_err / 2),
        center=(vx, y_center, vz),
        justify="right", width=0.05)
    # print beam_angle, ir1,ir2
    return ir1, ir2

  def setup_reflectors_blue(
          self, basebeam, theta1, clen, grating_spot_offset, pitch=1.5e6,
          lam0=1.053e-6, inside_retro_err=0, zeta=0):
    my = blue_compressor
    y_offset = my.y_offset
    grate = grating(
        my.GRATE, angle=theta1, center=(0, y_offset / 2, 0),
        pitch=pitch, order=1, justify=-.8, height=0.05, width=0.1)
    ...
    ir1, ir2 = self.setup_retro_standard(
        beam, vertex_dist, retro_beam_offset, inside_retro_err, y_offset / 2)
    vr1 = reflector(
        'vert retro 1', angle=0, center=vretvertex - yvec / 2, width=0.025,
        height=0.05, thickness=0.01)
    vr2 = reflector(
        'vert retro 2', angle=0, center=vretvertex + yvec / 2, width=0.025,
        height=0.05)
  ...
  def __init__(
          self, theta1, clen, lambda0, center=(0, 0, 0),
          angle=0, **extras):
    self.init(theta1, clen, lambda0, center, angle, extras)
  ...
  def init(self, theta1, clen, lambda0, center, angle, extras):
    ...
    optics = self.setup_reflectors_blue(
        basebeam, theta1, clen, grating_offset, lam0=lambda0,
    ...
    optics[
        my.IPERI1] = reflector(
        "input peri bot", angle=0, center=cs, width=.025)
    optics[
        my.IPERI2] = reflector(
        "input peri top", angle=0, center=cs + (0.0, -exit_height, 0),
        width=.025)

    optics[my.EXP1] = lens(
        "expander diverge", f=-.15).place_between(cs + (-.1, 0, 0), optics[my.IPERI1], -0.09)
    optics[my.EXP2] = lens(
        "expander re-coll", f=0.3).place_between(optics[my.IPERI2], (0, 0, 0), 0.037)
    ...
    optics[
        my.OPERI1] = reflector(
        "output peri top", angle=0, center=(0, my.y_offset, exit_z),
        width=.025)
    optics[
        my.OPERI2] = reflector(
        "output peri bot", angle=0, center=(0, exit_height, exit_z),
        width=.025)

    optics[
        my.TM1] = reflector(
        "output turn 1", angle=90, center=(0.30, exit_height, exit_z),
        width=.025)
...
def doit_bc(theta1, clen, lambda0, drawit=0):
  ...
  COMP = 'comp'
  comp = optics[COMP] = blue_compressor(
      theta1, clen, lambda0, center=system_center, angle=rotation)
  TM2 = 'turn2'
  optics[TM2] = reflector(
      "output turn 2", angle=0, center=(0.15, exit_height, -0.6),
      width=.025)
  ...
  optic_sys = composite_optic(
      "system", optics, sys_order, center=optics[START].center)
  ...
  if drawit:
    ...
    draw_trace(g, trace2)
    graphite.genOutput(
        g, 'QD', canvasname="Compressor layout", size=(600, 500))
...
class quadrupler(composite_optic):
  ...
  def __init__(self, center, from_optic, **extras):
    ...
    l2 = optics[my.L2] = lens(
        "quadrupler telescope 2", f=-0.150).place_between(ent, exit, 0.111).tilt_off_axis(4.0)
    # print l2.strength
    green = optics[my.DOUBLE] = null_optic(
        "green crystal").place_between(l2, exit, 0.1)
    optics[my.QUADRUPLE] = null_optic(
        "UV crystal").place_between(green, exit, 0.1)

    order = (my.L1, my.ENTRANCE, my.L2, my.DOUBLE, my.QUADRUPLE, my.EXIT)
    composite_optic.init(
        self, "quadrupler", optics, order, (0, 0, 0),
        center, 0, extras=extras)
    l1.place_between(from_optic, ent, -0.04)

  def entrance_optics_tags(self):
    # the alignment element in the quadrupler is _not_ the first element (the
    # lens)
...
def plotq(trace):
  ...
  colorline = graphite.PointPlot()
  colorline.lineStyle = graphite.LineStyle(
      width=1, color=graphite.red, kind=graphite.SOLID)
  colorline.symbol = None
  ...
  colorline = graphite.PointPlot()
  colorline.lineStyle = graphite.LineStyle(
      width=1, color=graphite.blue, kind=graphite.SOLID)
  colorline.symbol = None
  g.formats.append(colorline)
...
def show_table():
  ...
  optics[SPLIT] = split1 = reflector(
      "splitter1", center=(2 * inch, 5 * inch, 3 * inch))
  optics[IR] = irline = ir_line()

  comp = optics[COMP] = blue_compressor(
      38.5, 1.28, lambda0, center=compressor_entrance, angle=270)
  ...
  q = optics[QUAD] = quadrupler(
      center=(40 * inch, 2.5 * inch, 34.5 * inch),
      from_optic=comp[blue_compressor.TM1])
  comp.set_exit_direction(q)
  q.set_entrance_direction(comp)
  blue_sys = composite_optic(
      "blue_sys", optics, [START, SPLIT, BLUELINE, COMP, QUAD],
      (0, 0, 0),
      (0, 0, 0),
      0)
  ...
  ir_sys = composite_optic(
      "ir_sys", optics, [START, IR],
      (0, 0, 0),
      (0, 0, 0),
      0).clone()
  pointer = trace_path(ir_sys, mainbeam.clone())[-1]
  pointer.free_drift(1.0)
  optics[IR_COMP] = ircomp = ir_compressor(
      center=pointer.x0, angle=math.atan(0.2) / deg)
  ircomp.set_entrance_direction(irline)

  # now, align IZ
  ir_sys = composite_optic(
      "ir_sys", optics, [START, IR, IR_COMP],
      (0, 0, 0),
      (0, 0, 0),
      0).clone()
  pointer = trace_path(ir_sys, mainbeam.clone())[-1]
  pointer.free_drift(1.0)
  bemir = optics[BE_TURN] = reflector(
      "be turn mirror", center=pointer.x0, angle=45.0)
  ...
  ir_sys = composite_optic(
      "ir_sys", optics, [START, IR, IR_COMP, BE_TURN, IZ_SCREEN],
      (0, 0, 0),
      (0, 0, 0),
      0)
  ...
  if table_layout:
    ...
    if show_qd:
      graphite.genOutput(
          g, 'QD', canvasname="Compressor layout", size=(900, 500))
    if show_pdf:
