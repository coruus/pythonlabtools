...
class qparms:
  ...
  def __str__(self):
    if self.r is Infinity:
      ...
      rstr = "%.3f" % self.r
    return "lambda= %.4e, q= (%.3f %+.3fj), w=%.3f,  r=%s" % (
        self.lambda0, self.q.real, self.q.imag, self.w, rstr)
...
class optic:
  ...
  def __mul__(self, other):
    ...
    if isinstance(other, optic):
      ...
      return optic(
          mat, "Composite Optic:  \n" + Numeric.array_str(
              mat, precision=3, suppress_small=1))
    else:
...
class aperture(identityoptic):
  ...
  def __str__(self):
    if self.qparm is None:
      ...
      transmission = 1.0 - math.exp(-
                                    ((self.diameter / (2 * self.qparm.w)) ** 2))
...
class surface(optic):
  ...
  def __init__(self, r, n, name=None):
    optic.__init__(self, Numeric.array(
        ((1.0, 0.0),
         ((1.0 - n) / (n * r),
          1.0 / n))),
        "Dielectric surface%s: n'=%.3f r=%.3f" %
        (fixname(name),
         n, r))
...
def resonator(r1, r2, l, lambda0):
  ...
  return z0, w0, w0 * math.sqrt(1 + (zoff / z0) ** 2), w0 * math.sqrt(1 +
                                                                      ((l - zoff) / z0) ** 2), zoff - l / 2
...
class ztrace:
  ...
  def __init__(self, matlist, end_dist, q0):
    ...
    objectpos = []
    objects = []
    # mark out positions of optical components, backup up over probes
    for (dz, obj) in matlist:
      objectpos.append(zpos + dz)
      if not isinstance(obj, probe):
        zpos += dz
      objects.append(obj)
    zpos = 0.0
    opticindex = 0
