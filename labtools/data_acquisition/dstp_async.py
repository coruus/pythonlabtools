  ...
  class threaded_response_data(NATI_DSTP_DataServer):
    ...
    def __init__(self):
      self.__sender_thread = threading.Thread(
          target=self.__send_data_thread, name='DSTP sender')
    ...
    def __send_data_thread(self):
      ...
      # keep_running is inherited from the main server, conveniently
...
class DSTPServer(SocketServer.TCPServer, DataServer):
  ...
  def __init__(self):
    SocketServer.TCPServer.__init__(
        self, (self.connect_ip, self.connect_port),
        None)
    self.debug = 0
    self.socket_info = {}
  ...
  def serve(self):
    ...
    self.socket_info[
        self.socket] = {
        "addr": '127.0.0.1', "handle": self.new_connection, "name": 'main'}
    if self.debug:
          ...
          for sock in streams:
            try:
              ...
              if sock is self.socket:
                # ouch. someone closed or damaged our main socket
                self.keep_running = 0
...
if __name__ == '__main__':
  ...
  data.define_field(
      "test2", s, ("TestArray", array.array('d', [0])),
      writable=1)
  data.define_field(
      "test3", s, ("BooleanArray", array.array('b', [0])),
      writable=1)
