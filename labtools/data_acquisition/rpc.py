...
# Now there is, on receives. 2003.02.20 Marcus Mendenhall, Vanderbilt
# University marcus.h.mendenhall@vanderbilt.edu
...
class TCPClient(RawTCPClient):
  ...
  def __init__(
          self, host, prog, vers, portmap_proxy_host=None,
          portmap_proxy_port=PMAP_PORT, timeout_seconds=None):

    self.select_timeout_seconds = timeout_seconds
    if portmap_proxy_host is None:
      # use a proxy to get around firewalled portmappers
      portmap_proxy_host = host
    pmap = TCPPortMapperClient(
        portmap_proxy_host, portmap_proxy_port, timeout_seconds)
