...
class hp_8753e(vxi_11_connection):
  ...
  def setup_simple_sweep(
          self, min=1e4, max=3e9, count=1601, sweeptype='lin', power=10.0):
    # when switching frequencies, always disable transform mode and corrections
    if sweeptype == 'lin':
      s = "TIMDTRAN OFF;CORR OFF;LINFREQ;"
    else:
      s = "TIMDTRAN OFF;CORR OFF;LOGFREQ;"
    s += "POIN %d;STAR %.8e Hz;STOP %.8e Hz;POWE %.2f" % (
        count, min, max, power)
    self.lock()
    try:
      self.write(s)
  ...
  def enable_transform(self, tmin=-1e-8, tmax=1e-8, mode='impulse'):
    ...
    self.write("SETF;TIMDTRAN ON;%s;REAL;STAR %.8e;STOP %.8e;" %
               (s, tmin, tmax))
  ...
  def get_converted_data(self, command):
    ...
    if len(result) != count + 4:
      raise HP8753_Error(
          "Bad data string length:  expected %d, got %d" %
          (count + 4, len(result)))
  ...
  def generate_xaxis_data(self):
    ...
    try:
      info = [
          self.transaction(command)[2]
          for command
          in
          ["POIN?;", "STAR?;", "STOP?;", "LINFREQ?;", "LOGFREQ?;",
           "TIMDTRAN?;"]]
    finally:
      self.unlock()
    ...
    points = int(fpoints)
    basearray = numpy.array(
        range(points), numpy.float) * (1.0 / (points - 1))  # array on [0,1]
    ...
    if transform or linf:
      ...
      raise HP8753_Error(
          "Unknown x-axis mode... not linear, log, or time, and I don't handle segments yet")
  ...
  def average_and_wait(
          self, averages=1, wait_loop_time=1.0, max_wait_time=None):
...
if __name__ == '__main__':
  analyzer = hp_8753e(
      host="", device="gpib0,3", timeout=4000, device_name="network_analyzer",
      raise_on_err=1)
  print analyzer.idn
  analyzer.lock()
  try:
