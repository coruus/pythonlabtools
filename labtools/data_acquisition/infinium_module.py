...
class infinium_54825a(agilentscope):
  ...
  preamble_items = (
      ...
      ("xunits", int),
      ("yunits", int),
      ("bandpass-upper", float),
      ("bandpass-lower", float))
  ...
  def get_current_data(self, chan_list, max_points=None):
    ...
    for i in chan_list:
      ...
      err, reason, data = self.transaction(
          ":syst:head off;:wav:sour chan%d;form word;byt msbf;pre?;data?" % i,
          count=count)
      if not err:
        curve = data.find(";#")
  ...
  def average_mode(self, averages):
    self.write(
        """:acquire:mode etime;average 1;count %d;points auto;complete 100""" %
        averages)

  def realtime_mode(self, length):
    self.write(
        """:acquire:mode rtime;average 0;points %d;complete 100""" % length)

  def set_channel(
          self, chan, range, offset=0, coupling=agilentscope.dc,
    ...
    if lowpass:
      ...
      lp = 0
    setup = ":chan%d:probe %f;range %.0e;offs %.2e;inp %s;bwl %d;" % (
        chan, atten, range, offset, coupling, lp)
    self.write(setup)

  def set_edge_trigger(
          self, source, level=0, slope=1, coupling=agilentscope.dc,
          auto_trigger=1):
    ...
    if source != self.line:
      levelstr = "level %s,%.2e;edge:source %s;coup %s;slop %s;:trig:sweep %s" % (
          source, level, source, coupling, slope, mode)
    else:
...
if 1:
  infinium = infinium_54825a(
      host="", device="inst0", timeout=4000, device_name="infinium",
      raise_on_err=1)
else:
  infinium = infinium_emulated(
      host="", portmap_proxy_host='127.0.0.1', portmap_proxy_port=1111,
