...
class LabProDAQ(DAQ_device.DAQ_device):
  ...
  channel_modes = {
      'off': 0, 'auto': 1, '10volts': 2, '10amps': 3, 'resistance': 4,
      'period': 5, 'frequency': 6, 'count': 7, '5volts': 14, 'output_current':
      22}
  ...
  def set_input_channel_properties(
          self, chan=1, range=5, offset=0, mode='auto', converter=None):
    ...
    time.sleep(0.5)
    # figure out what it really did
    actconfig = self.lp.get_channel_setup(chan)
    # remember how we set it up for conversion, if it isn't being turned off
    self.channel_info[chan] = (mode, converter, actconfig)
    if not op:
      # if channel ends up off, remove it from list
      del self.channel_info[chan]
    pass

  def set_sample_properties(
          self, npoints=100, sampletime_seconds=1.0, averages=0,
          prestore_seconds=0, **kw):
    "set the block length, inter-sample time, and averageing for an acquisition. npoints=0 streams data"
    # at the entry, locals() just contains the argument list
    self.config_params.update(locals())
    self.configured = 0

  def set_trigger_properties(
          self, trig_channel=0, trig_level=0.0, trig_slope=1,
          trig_mode='immediate', **kw):
  ...
  def start_collection(self):
    if not self.configured:  # must send new config
      ...
      if cp.npoints:
        ...
        prestore = int(
            100.0 * cp.prestore_seconds /
            (cp.sampletime_seconds * cp.npoints))
        if prestore > 100:
      ...
      self.lp.setup_data_collection(
          samptime=cp.sampletime_seconds, numpoints=npoints, trigtype=trigtype,
          trigchan=cp.trig_channel, trigthresh=cp.trig_level,
          prestore=prestore, rectime=0, filter=0, fastmode=fastmode)

      # flag that this is done so it doesn't get repeated when nothing is
      # changed
      self.configured = 1
    else:
...
if __name__ == '__main__':
  ...
  def test():
    ...
    try:
      lpDAQ = LabProDAQ(lp)
      # channel 1 default configuration, on
      lpDAQ.set_input_channel_properties(1)
      lpDAQ.set_sample_properties(
          npoints=100, sampletime_seconds=0.01, prestore_seconds=.005)
