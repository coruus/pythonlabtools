...
try:
    ...
    def __init__(self, idProduct, device_index):
      ...
      try:
        ...
        if firstmsg.find("Found device") < 0:
          raise MeasurementComputingError(
              "USB Server could not connect to a MCC device at index %d" %
              self.device_index)

        self.idProduct = int(
            firstmsg[firstmsg.find("ID=") + 3:],
            16)  # which flavor of MCC device?
        self.__keep_running = 1
        self.status_monitor = threading.Thread(
            target=self.read_status, name='USB MCC status monitor')
    ...
    def read(self, maxlen=8, feature=0, send_command=1):
      ...
      while len(res) < actmax and tries < 20:
        try:
          ...
          if err[0] in (11, 29, 35):
            time.sleep(0.01)
            tries += 1
            continue  # just return empty data
          else:
            raise MeasurementComputingError(
                "USB server disconnected unexpectedly", sys.exc_info()[1].args)
      ...
      if actbytecount < 0:
        raise MeasurementComputingTimeout(
            "Probably USB Timeout, error %08lx" % actbytecount)
      elif actbytecount != maxlen:
        raise MeasurementComputingError(
            "Bad data length from MCC device:  requested %d, got %d" %
            (maxlen, actbytecount))

      # print len(res), locals()
      if flag != 0x00ffffff:
        raise MeasurementComputingError(
            "Bad packet header from MCC device: " +
            ("%04x %08x %08x" % (flag, tv_sec, tv_usec)))
    ...
    def read_status(self):
      ...
      while(self.__keep_running):
        try:
          ...
          if res:
            print >> sys.stderr, "MCC USB message: ", res
          if res.find("****EXITED****") >= 0:
            break  # if this thread dies, device has gone away somehow
        except IOError:
          if sys.exc_info()[1].args[0] in(
                  11, 29, 35):  # this error is returned on a nonblocking empty read
  ...
  class USB_Win32_libusb_mixin:
    ...
    server_executable_path = os.path.join(
        os.path.dirname(__file__),
        "MeasurementComputingServer.exe")
    ...
    def __init__(self):
      ...
      try:
        ...
        if firstmsg.find("No LabPro Found") >= 0:
          raise MeasurementComputingError(
              "USB Server could not connect to an MCC device at index %d" %
              self.device_index)
    ...
    def read(self, maxlen=8):
      ...
      while(len(res) < maxlen):
        # 8 bytes + 8 byte timestamp + + 4 byte count + 4 byte 0xffffffff flag
        while len(db) < 20:
    ...
    def read_status(self):
      ...
      while(self.__keep_running):
        ...
        if res:
          print >> sys.stderr, "MCC USB message: ", res
        if res.find("****EXITED****") >= 0:
          break  # if this thread dies, LabPro has gone away somehow
...
class MCC_Device(default_server_mixin):
  ...
  def write_memory(self, base=USER_MEMORY_BASE, data=''):
    tc = 0
    ld = len(data)
    # convert string to list for concatenation onto other list
    bd = map(ord, data)
    while(tc < ld):
      actcount = min(ld - tc, 4)
      self.write(
          [self.CBMEMWRITE, base & 0xff, (base >> 8) & 0xff, actcount] +
          bd[tc: tc + actcount])
      base += actcount
      tc += actcount
  ...
  def write_user_memory(self, data):
    assert len(
        data) < self.USER_MEMORY_SIZE - 2, "Too much data for user memory"
  ...
  def read_user_memory(self):
    ...
    # protect against junk if reading uninited memory
    bytes = min(struct.unpack(">H", lendata)[0], self.USER_MEMORY_SIZE - 2)
...
class MCC_Analog_Support:
  ...
  def setup_gain_list(self, channels, gains):
    ...
    changain = [(self.AD_DIRECT_MODE | c & 0x07 | g)
                for c, g in self.channel_gain_list]
  ...
  def get_burst_scan(self, max_trig_wait=None):
    ...
    for i in range(self.burst_scan_blocks):
      ...
      if index != i + 1:  # aack, packet out of sync
        raise MeasurementComputingError(
            "scan packet out of sync... expected %d, got %d" % (i + 1, index))
  ...
  def get_continuous_scan_packet(self, resync=0):
    ...
    # print err, readaddr, writeaddr, index
    # aack, packet out of sync
    if not resync and index != self.continuous_scan_packet_index:
      raise MeasurementComputingError(
          "scan packet out of sync... expected %d, got %d" %
          (self.continuous_scan_packet_index, index))
  ...
  try:
    ...
    if 1:
      mcc.setup_analog_scan(
          sweeps=1000, channels=(0, 1),
          gains=mcc.GAIN2_DIFF, rate=500)
      time.sleep(2)
    ...
    if 1:
      mcc.setup_analog_scan(
          sweeps=-1, channels=(0,),
          gains=mcc.GAIN2_DIFF, rate=1000)
      print mcc.actRate
      ...
      try:
        ...
        while actcount < countlimit:
          ...
          if res:
            # print Numeric.array_str(res[:,0], precision=3, suppress_small=1,
