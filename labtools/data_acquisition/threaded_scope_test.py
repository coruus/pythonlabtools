...
class scope_monitor_thread(vxi_11.device_thread):
  ...
  def __init__(
          self, scope, channels, data_wait_sleep=1.00, data_wait_loops=10,
          main_sleep=1.0, name="Device", debug=0):
    vxi_11.device_thread.__init__(
        self, scope, main_sleep=main_sleep, name=name)
    self.scope = scope
    self.data = None
  ...
  def get_data(self):
    try:
      ...
      scope.wait_for_done(
          sleep=self.data_wait_sleep, max_loops=self.data_wait_loops)
      self.waveform = Numeric.array(scope.get_current_data(self.channels))
      self.data_fail = 0
      dump_scpi_errors(scope, self.name + ": post-readout: ")
      scope.log_error(
          "thread %s got data, %d points" %
          (self.name, len(self.waveform[0])),
          self.connection().debug_info)
    except UserWarning:
      self.data_fail += 1
...
def test1():
  ...
  # if re-sending the original setup creates an error, the setup is
  # irrational and will be reset
  if dump_scpi_errors(scope):
    scope.write("*rst")
  ...
  try:
    ...
    scope.set_channel(
        1, range=5, offset=0, coupling=scope.ac, atten=10.0, lowpass=0,
        impedance=50)
    dump_scpi_errors(scope)
    scope.set_channel(
        2, range=0.1, offset=0, coupling=scope.dc, atten=1.0, lowpass=0,
        impedance=None)
    dump_scpi_errors(scope)
    scope.set_channel(
        3, range=0.1, offset=0, coupling=scope.dc, atten=10.0, lowpass=1,
        impedance=None)
    dump_scpi_errors(scope)
    scope.set_channel(
        4, range=5, offset=2, coupling=scope.dc, atten=1.0, lowpass=1,
        impedance=50)
    dump_scpi_errors(scope)
    ...
    scope.set_edge_trigger(
        1, level=0, slope=1, auto_trigger=1, coupling=scope.ac)
    dump_scpi_errors(scope)
    fast_trace = scope_monitor_thread(
        scope, (1,),
        name="Fast", main_sleep=60.0, data_wait_loops=100)
    dump_scpi_errors(scope)

    # and specifics for the other
    scope.set_edge_trigger(
        scope.line, level=0, slope=1, auto_trigger=1, coupling=scope.ac)
    timerange = 2e-2
    loops = 1
    plotskip = 1
    scope.set_timebase(range=2e-2, reference=scope.left, delay=-2e-9)
    scope.realtime_mode(2048)
    slow_trace = scope_monitor_thread(
        scope, (1, 2,),
        name="Slow", main_sleep=30.0)
  ...
  try:
    ...
    if 1:
      ...
      while(1):
        time.sleep(1)
        if not (fast_trace.running and slow_trace.running):
          break
