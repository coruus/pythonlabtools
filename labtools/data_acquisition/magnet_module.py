...
class scpi_magnet_supply(
        alarm_system.local_vxi_11, current_supply, scpi_power_supply):
...
if __name__ == "__main__":
  ...
  try:
    ...
    mymagchan = magnet_channel(
        "magnet_1", mymag, error_tol_absolute=0.03, error_tol_relative=0.005,
        settling_time=1, max_wiring_resistance=None,
        min_wiring_resistance=None, relay_channel_name="magnet_1_relay",
        max_switch_current=0.01)
    ...
    # dangerous in real code, OK for testing or shutting down
    alarm_system.clear_all_alarms()
    # do this so shutting down doesn't cause a panic!
    alarm_system.stop_critical_thread_monitor()
    # magnets.close()
