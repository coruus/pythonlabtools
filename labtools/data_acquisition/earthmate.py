...
class raw_earthmate:
  ...
  def __init__(self, portname):
    ...
    self.data_monitor = threading.Thread(
        target=self.read_data, name='gps data monitor')
    self.keep_running = 1
    ...
    while not done and (maxloops is None or loops < maxloops):
      ...
      try:
        ...
        if done:
          if pos > 0:
            pass
            # print "garbage in buffer, length %d %s" % (pos ,
            # repr(self.serial_buffer[:10]))
  ...
  def readmsg(self):
    ...
    if err:
      raise EarthmateBadData(
          "bad header checksum %04x: data is %04x %04x %04x %04x %04x" %
          tuple([err] + list(header)))
    if not count:
    ...
    if err:
      raise EarthmateBadData(
          "bad packet checksum %04x, message=%d, count=%d" %
          (err, header[1],
           count))
    return header, packet, times[0]

  def enable_message_queue(self):
    self.message_collector = threading.Thread(
        target=self.collect_messages_thread, name='gps message collector')
    self.message_queue = []
  ...
  def set_elev_mask(self, angle_degrees=5.0):
    self.send_msg(
        1212, 0,
        [self.next_sequence(),
         int(1000.0 * angle_degrees * math.pi / 180.0)])

  def set_pinned_position(self, flags):
    self.send_msg(
        1221, 0, [self.next_sequence(),
                  flags & 0x0f, 0, 0, 0, 0, 0, 0, 0])

  def set_nav_configuration(
          self, pinning_disable=0, smoothing_disable=0,
          held_altitude_disable=0, filtering_disable=0):
    flags = (
        bool(filtering_disable) << 3) | (
        bool(pinning_disable) << 2) | (
        bool(smoothing_disable) << 1) | bool(held_altitude_disable)
      ...
      while(1):
        try:
          ...
          if longform:
            ...
            print >> out, time.asctime(), '\t', time.asctime(
                time.localtime(times)), '\t', header[1]
