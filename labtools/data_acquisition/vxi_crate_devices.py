...
class e1413b(vxi_crate_device, tagged_data.array_device):
  ...
  def setup_device(self):
    ...
    try:
      ...
      try:
        ...
        pass
      self.write(
          "*cls;*sre 0;:stat:ques:enable #H0800;*ese 63;:init:cont off;")
      err, reason, setup_check = self.mav_transaction(
          ":diag:otd? (@100);:stat:ques:cond?")
      otdstat, qstat = [int(i) for i in setup_check.split(";")]
      self.log_error(
          "otd status = %d, questionable status = %04x" % (otdstat, qstat),
          self.debug_info)
      if otdstat:
        self.write(":diag:otd off,(@100,108);")
        self.log_error(
            "Waiting 2 minutes for otd to settle before recalibrating",
            self.debug_info)
        time.sleep(120)
      if qstat & 0x2000:  # configuration changed... must recalibrate
        self.log_error(
            "recalibrating a/d... this takes a while", self.debug_info)
        self.write("*cls;:func:volt:dc 10,(@100:155);:cal:set")
        done = 0
        # let cal get started, during which time system is basically hung
        time.sleep(30)
        self.write(":cal:set?")
        while(not (done & 16)):
          time.sleep(60)
  ...
  def monitor(self):  # override this since we are forever locked
    # adding one to the locklevel prevents us from unlocking until the very end
    self.lock()
    try:
      ...
      # and call normal monitor loop (which completely unlocks at the end)
      device_thread.monitor(self)

  # used by our device thread to read out data, overridden here due to
  # permanent lock
  def onepass(self):
    ...
    try:
      ...
      if count == self.scan_channels:
        self.save_read_data(
            array.array('f', list(struct.unpack(">%df" % count, result))))
        self.consecutive_failures = 0  # reset! got good data
        self.last_good_data_time = time.time()

      else:
        self.log_error(
            "Wrong length data on loop %d, len= %d" %
    ...
    if self.loop_count % 100 == 0:
      ...
      self.log_error(
          "Loop %d, recent timeouts=%d" %
          (self.loop_count, self.recent_timeouts),
          warn_level)
...
class e1328a(vxi_crate_device, tagged_data.array_device):
  ...
  def write_data(self, chanlist):
    ...
    # normally, we should already be locked by the time we get here
    self.lock()
    try:
      self.write(outstr)
      if self.debug_scpi:
    ...
    try:
      ...
      if len(words) == 3:
        for i in range(3):
          if words[i] < 0:
            words[i] += 2L << 31
        # now, put bytes in reasonable order! the words come back
        # reversed-endian w.r.t the byte number sent in a write
        w0, w1, w2 = struct.unpack(
            "<LLL", struct.pack(">LLL", words[0], words[1], words[2]))
        self.save_read_data(w2 << 64 | w1 << 32 | w0)

        # do digital debouncing by compaing ands and ors of history
        # bump data up one row
        self.deglitch_buffer[:-1] = self.deglitch_buffer[1:]
...
class glue(vxi_crate_device, tagged_data.dio_device):
  ...
  def __init__(self, slot=11):
    self.data_read_str = ":vxi:sel %d;reg:read? 4;write 8,0" % (
        slot * 8)  # read data and tweak watchdog
    self.data_write_str = ":vxi:sel %d;reg:write 6,%%d" % (
        slot * 8)  # read data and tweak watchdog
    # we really talk to slot zero to control this
    vxi_crate_device.init(self, 0, "glue board", "glueboard(%d)" % slot)
    tagged_data.dio_device.init(self)
  ...
  def setup_device(self):
    ...
    try:
      ...
      # preflight data write, and clear bits
      self.write(self.data_write_str % 0)
      # preflight a read and reset the watchdog
      self.transaction(self.data_read_str)
      self.check_scpi_errors()
    finally:
      self.unlock()
