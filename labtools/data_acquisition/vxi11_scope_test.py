...
try:  # if the persistent scope module exists, use it
  ...
  class hp54542(vxi_11.agilentscope):
    ...
    # key this device down to the serial-number level
    idn_head = "HEWLETT-PACKARD,54542C,US36040757"
  ...
  if 0:
    scope = hp54542(
        ...
        device="gpib0,7", timeout=4000, device_name="hp54542", raise_on_err=1)
  else:
    scope = hp54542(host="",
...
def dots(color):
  ...
  reddots.symbolStyle = graphite.SymbolStyle(
      size=1, fillColor=color, edgeColor=color)
  return reddots


def lines(color):
  reddots = graphite.PointPlot()
  reddots.lineStyle = graphite.LineStyle(
...
def test1():
  try:
    ...
    # do setup, which should always be done if the scope has been unlocked,
    # since someone else might have used it.
    if 1:
      scope.set_edge_trigger(
          2, level=0, slope=-1, auto_trigger=1, coupling=scope.ac)
      scope.set_timebase(range=timerange)
      scope.set_channel(
          2, range=5, offset=2.5, coupling=scope.dc, lowpass=0, impedance=None)
      scope.set_channel(
          3, range=0.1, offset=0, coupling=scope.dc, lowpass=0, impedance=None)
