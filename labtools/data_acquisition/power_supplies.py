...
class power_supply(subsystem):
  ...
  # mapping of logical channel numbers to physical on this device
  physical_channels = (1,)
  ...
  def write_field(self, channel_info, value_pair=(None, None)):
    ...
    # the first element of our info tuple is the physical channel
    self.write_data((channel_info[0], value_pair))

  def read_field(self, channel_info):
    # our channel_info is always a physical channel number followed by the
    # position information in self.scan
    return [self.scan[c] for c in channel_info[1:]]


class scpi_power_supply(
        power_supply, vxi_11_connection, gpib_utilities.gpib_device):
  ...
  def send_list(self, stringlist):
    self.lock()
    try:
      # send all non-blank strings with semicolons between them
      self.write(";".join([s for s in stringlist if s]))
    finally:
      self.unlock()
  ...
  def transact_list(self, stringlist):
    ...
    try:
      # send all non-blank strings with semicolons between them
      a = self.transaction(";".join([s for s in stringlist if s]))[2]
    finally:
      self.unlock()
  ...
  def write_data(self, channels):
    ...
    for phys_chan, (voltage, current) in channels:
      ...
      init.append(
          self.format_output(
              self.voltage_set_string, self.voltage_number_format, voltage))
      init.append(
          self.format_output(
              self.current_set_string, self.current_number_format, current))
  ...
  def close(self):
    ...
    for c in self.channels:
      # convert virtual channels to real channels
      c = self.physical_channels[chan]
      if self.channel_select_string:
...
class analog_power_supply(power_supply):
  ...
  def write_data(self, channels):
    ...
    assert c == 0 and (
        voltage is None or current is None), "Analog supplies only have one channel and only one settable parameter!"
    self.save_setpoint(0, (voltage, current))
    if voltage is None:
      self.data_system[
          self.da_name] = current * self.da_transconductance + self.da_v0
    else:
      self.data_system[
          self.da_name] = voltage * self.da_transconductance + self.da_v0
...
class current_supply:
  ...
  def write_field(self, channel_info, current):
    ...
    # the first element of our info tuple is the physical channel
    self.write_data((channel_info[0], (None, current)))
...
class voltage_supply:
  ...
  def write_field(self, channel_info, voltage):
    ...
    # the first element of our info tuple is the physical channel
    self.write_data((channel_info[0], (voltage, None)))
