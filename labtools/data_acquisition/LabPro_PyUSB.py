...
class PyUSB_mixin:
  ...
  def setup_serial(self, port_name=None):
    ...
    try:
      ...
      for bus in usb.busses():
        for dev in bus.devices:
          if dev.idVendor == self.idVendor and dev.idProduct == self.idProduct:
            matchcount += 1
            matchdev = dev
            if matchcount == self.device_index:
              break
      ...
      if not matchdev or matchcount != self.device_index:
        raise IOError(
            "no USB LabPro found with requested index %d" % self.device_index)
    ...
    try:
      ...
      while self.__keep_running:
        ...
        try:
          ...
          continue
        stop_time = time.time()
        if stop_time - start_time > 9.5:
          continue  # probably a timeout, just keep going
  ...
  def read(self, maxlen=None, mode=None):
    ...
    while (not maxlen or (maxlen and len(res) < maxlen)) and self.__keep_running:
      ...
      if not mode and data.find('\r'):
        break  # ascii data terminator
    if not mode:
      zp = res.find('\0')
      if zp >= 0:
    ...
    if not self.__keep_running:
      raise IOError(
          "USB Labpro index %d closed, reading still going on" % self.device_index)
    return res
      ...
      while(1):
        ...
        try:
          ...
          try:
            ...
            print >>f, "%s\t%.3f\t%.3f\t%.1f" % (
                time.asctime(),
                grabtime - start_time, value, value * 31.47 - 31.74)
