...
# handle named data fields and lazy/immediate writing with this mix-in
class dio_device:
  ...
  def write_data(self, data, mask):
    ...
    try:
      if mask or self.lazy_write_mask:
        ...
        # first look at lazy changes
        changemask = (self.lazy_data ^ ld) & self.lazy_write_mask
        ld ^= changemask
        # map of changed bits, with non-lazy vs. lazy conflicts favoring
        # non-lazy
        changemask ^= (data ^ ld) & mask
        # map changes into latched value
        self.latched_output_data ^= changemask
  ...
  def bind_field_info(self, info=None):
    if info:
      ...
      startbit, mask = 0, 0
    return self.read_field, self.write_field, self.write_field_lazy, (
        startbit, mask)
...
class tagged_data_system:
  ...
  def define_field(self, global_name, device, device_info, writable=0):
    ...
    reader, writer, lazy_writer, bound_info = device.bind_field_info(
        device_info)
    if not writable:
...
# go ahead and create one of us globally, since this is probably the
# desired behavior
