...
__all__ = [
    "pack_object", "pack_object_completely", "parse_composite_object",
    "parse_typelist_and_string", "TimeoutError", "ClosedError",
    "NATI_DSTP_StreamHandler", "NATI_DSTP_DataServer", "UseNumericArray",
    "get_payload_from_string"]
...
def getarray(objd, string):
  ...
  assert not atype in (
      compositetype, stringtype), "No variable-size-element arrays yet"
  ...
  if prefer_Numeric_arrays:
    ...
    # do this after byteswapping, which only array.array knows how to do
    data = Numeric.array(data, data.typecode)
...
def pack_object(obj):
  ...
  this = type(obj)
  assert this in objectpackers, "Attempting to pack unknown object: " + \
      str(obj)
...
def parse_composite_object(string, max_items=None):
  ...
  for i in range(count):
    ...
    if objtype == compositetype or objtype == arraytype:
      # just save it raw for later descent, if needed
      objdata = string[:chunklen]
    else:
      objdata = ''
...
def parse_typelist_and_string(typelist, string):
  ...
  for objt, objd in typelist:
    if objt == compositetype:
      # pass a synthetic string down for recursion
      v, leftovers = parse_composite_object(objd + string)
...
class NATI_DSTP_StreamHandler:
  ...
  def listen(self, string, req, payload):
    ...
    # first send usual connect string
    self.respond(pack_object_completely([6, fullrequest[1], fullrequest[2]]))
  ...
  commandhandlers = [
      None,  blackhole, firstecho, goodbye, listen, delete_listener, store]

  def handle_one_request(self, sock):
    r = self.request
    try:
      # if we got here, select already says we have data, so be impatient
      line = recvfrag_with_timeout(r, timeout_seconds=self.read_timeout)
    except TimeoutError:
      return  # huh!?, we should have had data!

    # just for databasing, don't try to parse payload (3rd item in object)
    req, payload = parse_composite_object(line, max_items=2)
    command = req[0]
...
class NATI_DSTP_DataServer:
  ...
  def __init__(self):
    # since this is a mix-in, and 'data' is pretty generic, make it private
    self.__data = {}
    self.debug = 0
  ...
  def store(self, name, packed_string_or_tuple, force=0, never_send=0):
    ...
    if not me:  # add new key to dictionary
      # list of listeners,  payload, and other information
      d[name] = me = [[], '', {}]

    # a tuple of (header, data) needs repacking with correct length
    if type(packed_string_or_tuple) is types.TupleType:
  ...
  def echo_data(self, me):
    ...
    for target in listeners:  # the key is the value, no need to retrieve
      try:
        ...
        if self.debug:
          self.log_exception(
              "Disconnecting device from server due to exception sending data")
  ...
  def bind_field_info(self, field_info):
    ...
    # write a value to create the field
    self.store(name, (objt, objd), never_send=1)
  ...
  def write_field(self, name, value):
    ...
    assert objt == structure, "Incompatible DSTP data in field: " + str(
        name) + ",  expected: " + str(sample) + ", got: " + str(value)
    self.store(name, (objt, objd))

  def read_field(self, name):
    # last stored item is actualy payload
    return parse_composite_object(self.__data[name.upper()][1])[0][-1]
...
def get_payload_from_string(string):
  # last stored item is actualy payload
  return parse_composite_object(string)[0][-1]
...
if __name__ == '__main__':
  def raw_unpacker_test():
    ...
    data = [92, 0, 0, 0, 1, 8, 5, 0,  # 92 byte composite object with 5 elements
            ...
            # element 4, composite with 2 elements, descriptor length=20 bytes
            20, 0, 0, 0, 1, 8, 2, 0,
            6, 0, 0, 0, 0, 3,  # u32
            6, 0, 0, 0, 0, 3,  # u32
            6, 0, 0, 0, 0, 3,  # u32
            ...
            # 4byte length, string, pad byte
            11, 0, 0, 0, 65, 66, 67, 68, 69, 97, 98, 99, 100, 101, 102, 0,
            # u32, u32 data for embedded composite object
            123, 0, 0, 0, 234, 0, 0, 0,
            0, 0, 1, 0  # u32
            ]
  ...
  def packer_unpacker_test():
    ...
    o = pack_object_completely(
        [array.array('h', [1, 2, 3, 4, 5, 4, 3, 2, 1, 0, -1])])
    print `o`
    print parse_composite_object(o)[0]
