...
class Junk_OneWayAbortClient(RawTCPClient):
  ...
  def do_call(self):
    ...
    # put a valid return value into the unpacker
    self.unpacker.reset('\0\0\0\0')


class VXI_11_Error(IOError):
  vxi_11_errors = {
      0: "No error", 1: "Syntax error", 3: "Device not accessible", 4:
      "Invalid link identifier", 5: "Parameter error", 6:
      "Channel not established", 8: "Operation not supported", 9:
      "Out of resources", 11: "Device locked by another link", 12:
      "No lock held by this link", 15: "IO Timeout", 17: "IO Error", 21:
      "Invalid Address", 23: "Abort", 29: "Channel already established",
      "eof": "Cut off packet received in rpc.recvfrag()",
      "sync": "stream sync lost", "notconnected": "Device not connected"}
...
# exceptions having to do with multiple use which might get better
class VXI_11_Transient_Error(VXI_11_Error):
  pass
...
class vxi_11_connection:
  ...
  # by default, this class uses two-way aborts, per official vxi-11 standard
  OneWayAbort = 0
  ...
  def fancy_log_error(self, message, level=debug_error, file=None):
    if level <= self.debug_level:
      ...
      level_str = (
          "**INFO*****", "**ERROR****", "**WARNING**", "**DEBUG****")[level]
      if file is None:
        file = sys.stderr
  ...
  def log_exception(self, main_message='', file=None):
    self.log_error(
        main_message + traceback.format_exception_only(
            *(sys.exc_info()[: 2]))[0],
        self.debug_error, file)

  def __init__(
          self, host='127.0.0.1', device="inst0", timeout=1000,
          raise_on_err=None, device_name="Network Device", shortname=None,
  ...
  def command(self, id, pack, unpack, arglist, ignore_connect=0):
    ...
    # in a multi-threaded environment without needed a full vxi-11 lock to
    # make it safe
    if threads:
    ...
    try:
      try:
        result = self.core.make_call(
            id, arglist, self._list_packer, self._list_unpacker)
  ...
  def do_timeouts(self, timeout, lock_timeout, channel=None):
    ...
    if channel:
      # convert ms to sec, and be generous on hard timeout
      channel.select_timeout_seconds = 0.5 + \
          1.5 * max(timeout, lock_timeout) / 1000.0
  ...
  def abort(self):
    ...
    if self.abort_channel is None:
      return 0

    # convert to seconds
    self.abort_channel.select_timeout_seconds = self.timeout / 1000.0
    try:
      err = self.abort_channel.make_call(
          1, self.lid, self.abort_channel.packer.pack_int, self.abort_channel.
          unpacker.unpack_int)  # abort
    except EOFError:
  ...
  def disconnect(self):
    if self.connected:
      try:
        # execute destroy_link
        err, = self.command(23,  "id", "error", (self.lid,))
      except:
      ...
      self.lid = None
      self.core.close()
      if self.abort_channel is not None:
        self.abort_channel.close()
  ...
  def write(self, data, timeout=None, lock_timeout=0):
    ...
    while base < end:
      ...
      err, count = self.command(
          11, "write", "write",
          (self.lid, timeout, lock_timeout, flags, data[base: base + xfer]))
      if err:
        break
      base += count
    return err, base
  ...
  def read(self, timeout=None, lock_timeout=0, count=None, termChar=None):
    ...
    # wait for END or CHR reason flag or count
  ...
  def generic(self, code, timeout, lock_timeout):
    ...
    err, = self.command(
        code, "generic", "error", (self.lid, flags, timeout, lock_timeout))
  ...
  def read_status_byte(self, timeout=None, lock_timeout=0):
    ...
    err, status = self.command(
        13, "generic", "read_stb", (self.lid, flags, timeout, lock_timeout))
  ...
  def lock(self,  lock_timeout=0):
    ...
    if self.use_vxi_locking and self.locklevel == 0:
      ...
      try:
        if self.mux:
          self.mux.lock_connection(self.global_mux_name)
        try:
          err, = self.command(
              18, "lock", "error", (self.lid, flags, lock_timeout))
        except:
          if self.mux:
    ...
    try:
      if self.use_vxi_locking and self.locklevel == 0:
        try:
          ...
          if self.mux:
            # this cannot fail, no try needed (??)
            self.mux.unlock_connection(self.global_mux_name, priority)
      elif priority and self.mux:
        # even on a non-final unlock, a request for changed priority is always
        # remembered
  ...
  def unlock_completely(self, priority=0):
    ...
    if threads:
      # make sure we have the threadlock before we try a (possibly failing)
      # full lock
      self.threadlock.acquire()
    try:
      self.lock()  # just to be safe, we should already hold one level of lock!
    except VXI_11_Locked_Elsewhere:
      # this is often called on error cleanup when we don't already have a
      # lock, and we don't really care if we can't get it
      pass
    except VXI_11_Error:
    ...
    if threads:
      ...
      # unwind to single lock the fast way, and make sure this variable
      # really existed, to shield against internal changes
    self.locklevel = 1  # unwind our own counter, too
    try:
      self.unlock(priority)
    except VXI_11_Device_Not_Locked:
      # if we couldn't lock above, we will probably get another exception here,
      # and don't care
      pass
    except VXI_11_Transient_Error:
      self.log_exception(
...
class device_thread:
  ...
  def __init__(self,  connection, main_sleep=1.0, name="Device"):
    ...
    self.__thread = None
    # make a new copy to avoid a possible circular reference
  ...
  def set_connection(self, connection):
    ...
    # this allows the device_thread to be used as a clean mix-in class to a
    # vxi_11 connection
  ...
  def monitor(self):
    ...
    while(self.run):
      try:
        self.onepass()
        # wait until timeout or we are cancelled
        self.__wait_event.wait(self.main_sleep)
