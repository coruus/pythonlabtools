...
def lines(color):
  ...
  reddots.lineStyle = graphite.LineStyle(
      width=0.5, color=color, kind=graphite.SOLID)
  reddots.symbol = None
  return reddots
...
def test1():
  try:
    ...
    scope.set_edge_trigger(
        4, level=0.9, slope=1, auto_trigger=0, coupling=scope.dc)
    scope.set_channel(
        1, range=1, offset=0, coupling=scope.ac, atten=10.0, lowpass=0,
        impedance=None)
    scope.set_channel(
        2, range=5, offset=0, coupling=scope.dc, atten=1.0, lowpass=0,
        impedance=None)
    scope.set_channel(
        3, range=1, offset=1.2, coupling=scope.dc, atten=10.0, lowpass=1,
        impedance=None)
    scope.set_channel(
        4, range=1, offset=0.4, coupling=scope.dc, atten=1.0, lowpass=0,
        impedance=50)
