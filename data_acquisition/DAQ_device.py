...
class DAQ_device:
  ...
  # override this if some more interesting thread handler is needed e.g. one
  ...
  def set_sample_properties(
          self, npoints=1, sampletime_seconds=1.0, averages=None,
          prestore_seconds=0.0):
    "set the block length, inter-sample time, and averageing for an acquisition"
    pass

  def set_trigger_properties(
          self, trig_channel=1, trig_level=0.0, trig_slope=1,
          trig_mode='immediate', **keywords):
  ...
  def streamer_thread(self):
    ...
    while(self.__streaming_state != self.__stop_streaming):
      ...
      if self.__streaming_state == self.__streaming_paused:
        continue  # ignore new data if paused
  ...
  def collect_streaming_data(
          self, queue_overflow_action=queue_overflow_drop_old,
          max_queue_len=100):
