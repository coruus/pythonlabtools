...
class StepperChannel:
  ...
  def __init__(
          self, owner, owner_channel=1, steps_per_eng_unit=(1, 1),
          steps_per_encoder_step=(1, 1)):
    ...
    # set a flag for whether we know how to encode
    self.has_encoder = owner.has_encoder(owner_channel)
  ...
  def get_encoded_position(self):
    ...
    return self.compute_engineering_position_from_stepcount(
  ...
  def update_steps(self):
    ...
    # make sure that gratuitously repeated 'go' calls don't mess up our
    # settings
    self.next_steps = 0
  ...
  def setup_move_relative(self, engineering_pos_change):
    ...
    self.setup_move_absolute(
        self.current_eng_pos + engineering_position_change)
...
# if this happens, the actual subclass had better provide a real array class!
except:
  _myarray = None
...
class StepperCluster:
  ...
  def compute_steps_from_absolute_position(self, position):
    current_pos = self.array(
        [chan.get_current_position() for chan in self.channels])
    self.next_steps = (
...
class RawNorbergStepper(StepperOwner):
  ...
  # in this case, it also includes information about the numerical channel
  # number when a readback is done
...
if __name__ == '__main__':
  ...
  # The default Norberg setup runs 3200 microsteps/turn on the motor, so
  # 6400 steps is 1 mm
  # program in millimeters
  x = StepperChannel(a, owner_channel=a.x_axis, steps_per_eng_unit=(6400, 1))
  y = StepperChannel(a, owner_channel=a.y_axis, steps_per_eng_unit=(6400, 1))
  # tell system that 0 (current) step count is at 5 engineering units
  x.set_encoder_offset(5.0)
