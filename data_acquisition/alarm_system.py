...
__all__ = [
    "register_serious_alarm_handler", "register_fatal_alarm_handler",
    "register_warning_alarm_handler", "clear_all_alarms", "fatal_alarm",
    "serious_alarm", "warning_alarm", "start_critical_thread_monitor",
    "stop_critical_thread_monitor", "critical_thread", "local_vxi_11"]
    ...
    if isinstance(level, minlevel):
      ...
      if ref:
        try:
          ...
          alarm_subsystem.log_traceback(
              "Software failure: Alarm handler failed. Shutting down.")
...
class critical_thread(threading.Thread):
  ...
  def run(self):
    try:
      ...
      alarm_subsystem.log_traceback(
          "Uncaught exception in critical thread: " + self.getName())

    if _threadmonitor_keep_running:  # we are still critical!
      alarm_subsystem.log_error(
          "A Critical thread has died: " + self.getName())
...
class local_vxi_11(subsystem, vxi_11_connection, device_thread, gpib_device):
  ...
  # set this if device is on Agilent E5810 bridge, which doesn't abort
  # correctly
  using_e5810 = 1
