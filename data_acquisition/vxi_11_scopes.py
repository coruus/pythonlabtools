...
class oscilloscope(vxi_11_connection):
  ...
  def parse_binary_data(self, data):
    ...
    # print bdata[:100]
    fdata = (_numeric.array(
        bdata, _numeric_float) - self.preamble["YREF"]) * self.preamble["YINC"] + self.preamble["YORG"]

    self.xaxis = (
        _numeric.array(range(len(fdata)),
                       _numeric_float) - self.preamble["XREF"]) * self.preamble[
        "XINC"] + self.preamble["XORG"]
    return fdata
...
class tekscope(oscilloscope):
  ...
  def get_current_data(self, max_points=None):
    ...
    err, reason, data = self.transaction(
        "wfmo:byt_nr 2;:data:enc ribinary;:wavf?",  count=count)
    if not err:
...
class agilentscope(oscilloscope):
  ...
  def get_current_data(self, chan_list, max_points=None):
    ...
    for i in chan_list:
      ...
      err, reason, data = self.transaction(
          "syst:head off;:wav:sour chan%d;:wav:form word;:wav:pre?;:wav:data?" % i, count=count)
      if not err:
        curve = data.find(";#")
        self.parse_preamble(data[:curve])
  ...
  def average_mode(self, averages):
    self.write(
        """:tim:sample rep;:acquire:type average;points 500;count %d;complete 100;""" % averages)

  def realtime_mode(self, length):
    self.write(
        """:tim:sample real;:acquire:type norm;points %d;count 1;complete 100;""" % length)
  ...
  def set_channel(
          self, chan, range, offset=0, coupling=oscilloscope.dc,
          impedance=None, atten=1.0, lowpass=None):
    ...
    if lowpass:
      ...
      lp = 0
    setup = ":chan%d:probe %f;range %.0e;offs %.2e;coup %s;hfr %d;" % (
        chan, atten, range, offset, coupling, lp)
    self.write(setup)

  def set_edge_trigger(
          self, source, level=0, slope=1, coupling=oscilloscope.dc,
          auto_trigger=1):
    ...
    self.write(
        "trig:sour %s;slop %s;level %.2e;coup %s;:tim:mode %s" %
        (source, slope, level, coupling, mode))

  def digitize(self, chan_list=(1,)):
    # _must_ have the *esr? to clear old status bits!
    self.transaction(":stop;*esr?;*ese 1;*cls;")
    # drop the final comma from a channel list
    cl = ((len(chan_list) * "chan%d,") % chan_list)[:-1]
...
class hp54542(agilentscope):
  ...
  def parse_binary_data(self, data):
    ...
    # trim off the strange time-tag data before passing the buck
    if self.preamble["type"] == 4:
  ...
  def get_time_tags(self, max_grab_block=32):
    ...
    if shots > max_grab_block:
      for grab_block in range(max_grab_block, 8, -1):
        if shots % grab_block == 0:
          # try to intelligently factor the transfer down as small as 10, with
          # 9 being a bailout
          break

      if grab_block == 9:
        # no reasonable factoring, just allow individual moves at end
        grab_block = max_grab_block
    else:
      grab_block = shots
    ...
    for i in range(0, shots - (grab_block - 1), grab_block):
      result = self.transaction(
          com % tuple(range(i + 1, i + 1 + grab_block)))[2]
