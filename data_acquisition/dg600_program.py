...
FREEZE_BIT = 0x100
# bit set to pass to ADC indicating DQing valid after pulse
DID_PULSE_BIT = 0x10000
...
def parse_timing_files(files=("clock_timing.dat.txt",)):
  ...
  for fn in files:
    f = open(fn, "r")
    linelist = [
        (l, l.split('\t')) for l in[l.strip() for l in f.readlines()]
        if(l and l != "**END**" and l[0] != "#")]
    f.close()
    try:
      sigtable += [(int(s[0]), int(s[1]), int(s[2]))
                   for rawline, s in linelist]
    except ValueError:
  ...
  # sort into increasing time for edges
  sigtable.sort(lambda a, b: cmp(a[1], b[1]))
  state_word = 0L
  edges = len(sigtable)
  tr = []  # real transition list
  # do this twice, first to discover the final state, and then to measure
  ...
  for loop in (1, 2):
    ...
    for n in range(edges):
      ...
      if state:
        ...
        state_word &= ~mask
      # last edge or time change gets written out on second pass
      if loop == 2 and (n == edges - 1 or sigtable[n + 1][1] != time_offset):
        if n == edges - 1:
          dtime = 100
        else:
          t1 = sigtable[n + 1][1]
...
# this is a private procedure to generate_timing_setup
def generate_dg600_table(name, data):
...
def generate_timing_setup(auxfiles, kly_sub):
  ...
  # value of kicker bit between pulses
  div_kick_idle_state = (main_pattern[-1][0] & KICK_BIT_MASK)
  div_kick_idle_state_mask = bool(div_kick_idle_state) * DIV_KICK_MASK

  # make sure pattern stored has div kicker idling properly according to
  # kicker idle requirements
  for i in main_pattern:
    i[0] = (i[0] & ~DIV_KICK_MASK) | div_kick_idle_state_mask

  # final state at end of pulse / start of next pulse
  start_bits = main_pattern[-1][0]

  idle_pattern = [[start_bits | FREEZE_BIT, 1000], [start_bits, 1000000]]
  pulse_pattern = [
      [start_bits | FREEZE_BIT, 1000],
      [start_bits | DID_PULSE_BIT, 1000000]]
  ...
  outstr += ("TABLE 'KICK', %d ,'NOKICK', %d,'DIVKICK', %d, 'SYNC', %d\r" %
             (total_pattern_length, total_pattern_length, total_pattern_length,
              idle_size))  # define tables
  # put low 16 bits out on high-bit connector for mechanical convenience
  outstr += "FIELD 'bitorder','p16-p31,p0-p15'\r"
  ...
  nopulse = [
      ((pattern & (~(KICK_MASK | DIV_KICK_MASK))) |
       (start_bits & (KICK_MASK | DIV_KICK_MASK)),
       dt) for pattern, dt in main_pattern]
  ...
  divkick = [
      ((pattern & (~DIV_KICK_MASK)) |
       (bool(pattern & KICK_BIT_MASK) * DIV_KICK_MASK),
       dt) for pattern, dt in main_pattern]
...
def setup_clock(desired_freq, kick_sub, train_length=0, auxfiles=()):
  ...
  if(kly_sub != last_kly_sub):
    # if the kly_sub has changed, we need to load new tables
    tablestr = generate_timing_setup(auxfiles, kly_sub)
  else:
    tablestr = ''
  ...
  if not train_length:
    ...
    # note that at divider ratios below 2 Hz, divide-by-N clock runs at kicker
    # speed (i.e. slowly)
    # blank unkicked pattern string completely if kick_sub <= 1
    nokickstr = bool(kick_sub > 1) * (", 'NOKICK', %d" % (kick_sub - 1))

    if r2 > 1:  # generate sequence of pulses without divide kicker
      outstr += "SEQUENCE ML %d, %d %s, 'KICK',1\r" % (ml, r2 - 1, nokickstr)
      ml += 1

    # generate final set with divided kicker on
    outstr += "SEQUENCE ML %d, %d %s, 'DIVKICK',1\r" % (ml, 1, nokickstr)
    ml += 1
