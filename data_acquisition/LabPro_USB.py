...
# NOTE: this requires a copy of a LabProUSBServer executable in the same
# directory as this file.  See comments in LabProUSBServer.c for compile
# info.
...
class USB_data_mixin:
  ...
  def get_data_binary(
    ...
    # note... in USB mode, LabPro doesn't use checksums, so this is modified
    # from the usual
    if points is None:
      points = self.get_system_config()['dataend']
    self.command('data_control', chan, 0, 0, 0, 1)
    self.send_string('g')
    # round up to next 64-byte block, since LabPro always returns 64-byte
    # multiples in USB
    chunklen = (2 * points + 63) & (-64)
    s = ''
    empties = 0
    maxloops = int(max_wait_time / loop_sleep_time)
    while(empties < maxloops and len(s) < chunklen):
      time.sleep(loop_sleep_time)
      # mode=1 stops stripping of nulls from end of 64 bytes packets
      newdata = self.read(chunklen - len(s), mode=1)

      if not s and newdata:
        # remember when transmission started, in case someone cares
        self.data_transmission_start = self.data_timestamp
  ...
  def get_data_binary_realtime(self, channels=1):
    ...
    # binary, don't chop zeros, and new read routine always returns multiples
    # of 64
    s = self.read(mode=1)
    if not s:
...
try:
    ...
    def read_status(self):
      ...
      while(self.__keep_running):
        try:
          ...
          if res:
            print >> sys.stderr, "LabPro USB message: ", res
  ...
  class USB_libusb_mixin(USB_Mac_mixin):
    ...
    server_executable_path = os.path.realpath(
        os.path.join(os.path.dirname(__file__), "LabProUSBServer"))
...
if __name__ == '__main__':
  ...
  def monitor_humidity(
          filepath="/Users/marcus/Public/humidity.txt", sample_period=30.0):
    ...
    try:
      ...
      # set up 0-5 volts readback for humidity sensor
      lp.setup_channel(chan=2, operation=14, equation=0)
      ...
      while(1):
        ...
        try:
          ...
          try:
            ...
            print >>f, "%s\t%.1f\t%.3f\t%.1f" % (
                time.asctime(),
                grabtime, value, value * 31.47 - 31.74)
          ...
          badloops += 1
          if badloops >= 5:
            raise  # re-raise on 5th consecutive failure
  ...
  if sys.platform == "darwin":
    ...
    if not "DISPLAY" in os.environ:
      # in case we want to use X and haven't set it up.
  ...
  class graphxy(graph.graphxy):
    ...
    def display(self, file):
      ...
      os.system(
          'rm %s.* %s.*' %
          (self.texrunner.texfilename, self.latex.texfilename))
  ...
  def rt_test():
    ...
    try:
      ...
      g = graphxy(
          width=20, y=graph.linaxis(title='volts (V) \\& current (mA)'),
          x=graph.linaxis(title='time (sec)', min=0))
