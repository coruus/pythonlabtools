...
class magnet_system(subsystem):
  ...
  def add_supply(self, magnet_supply):
    if magnet_supply not in self.magnet_supplies:
      # magnet supplies may be added multiple times, and ignored, to make
      # multi-channel supplies easy
  ...
  def monitor_thread(self):
    while self.keep_running:
      # take a breath between scans, so we don't thrash if there are no
      # supplies
      time.sleep(1.0)
      ...
      for chan in current_magnets:
        if not self.keep_running:
          break  # quit early if we are stopped
        # return set, rdbk, volts, scaled_error, error_summary,
        # tolerance_worry, tolerance_big_trouble, resistance, resistance_low,
        # resistance_high
        ...
        if info["error_summary"]:
          ...
          errordescription = '\n'.join(
              [i + ": " + str(info[i]) for i in info.keys()])
          if (howbig == 0 and fatal) or when < (time.time() - self.alarm_minimum_interval):
            # log right now if error has escalated, otherwise log at most once
            # an hour
            self.log_error(
                "Magnet out of tolerance: " + chan.name + "\n" + errordescription)
            self.alarms[chan.name] = (time.time(), fatal)

        if fatal:
          serious_alarm(
              "magnet " + chan.name + " way off!\n" + errordescription)
        elif info["error_summary"] and not fatal:
          warning_alarm(
              "magnet " + chan.name + " drifting off!\n" + errordescription)
...
class magnet_channel:
  ...
  def set_value(self, no_channel_info, value):
    # we are never settled while waiting to switch
    self.settled_time = time.time() + 10.0 + self.settling_time
  ...
  def read_magnet_info(self, no_channel_info=None):
    ...
    if self.relay_channel_name:
      # +1 if relay=0, -1 if relay=1
      sign = (1 - 2 * data[self.relay_channel_name])
      iset, rdbk = iset * sign, rdbk * sign

    scaled_error = (
        rdbk - iset) / math.sqrt(iset * iset * self.error_tol_relative2 + self.error_tol_absolute2)
    ...
    if settled and(
            self.min_wiring_resistance or self.max_wiring_resistance) and(
            rdbk * rdbk > 25.0 * self.error_tol_absolute2):
      # check resistance if current is at least 5 times absolute current
      # tolerance
      resistance = abs(volts / rdbk)
      resistance_low = self.min_wiring_resistance and(
          resistance < self.min_wiring_resistance)
      resistance_high = self.max_wiring_resistance and(
          resistance > self.max_wiring_resistance)
    else:
  ...
  def handle_switch(self):
    ...
    while(1):
      ...
      if self.relay_channel_name:  # handle sign switching
        ...
        if not decay_start:
          # if we're not already in the process of decaying, check for sign
          # changes
          # 0 -> positive, 1 -> negative
          currentsign = data[self.relay_channel_name]
          # now, see if the relay has to switch, and if so start the timing
          # sequence
          if (target_newsign != currentsign):
            self.settled_time = time.time() + 10.0  # long in the future
            # start decay on sign switch
            self.channel_writer(self.channel_info, 0.0)
            decay_start = 1
        ...
        while decay_start and self.keep_running and not event.isSet() and abs(self.channel_reader(self.channel_info)[1]) > self.max_switch_current:
          # keep this long in the future
          self.settled_time = time.time() + 2.0 * self.settling_time + 10.0
          event.wait(self.settling_time)
          self.update_data()  # read it now
        if not self.keep_running:
          break
        if event.isSet():
          # someone tweaked us again, so start over, otherwise it timed out
          continue
      ...
      # now we are really settling
      self.settled_time = time.time() + self.settling_time
