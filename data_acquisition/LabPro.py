...
def init_port_memory():
  ...
  # _highspeedports is a persistent memory of which serial ports have had
  # their LabPros set to high speed
  try:
    a = _highspeedports
  except:
    _highspeedports = []
...
class RawLabPro:
  ...
  commands = {
      'reset': 0, 'channel_setup': 1, 'data_collection_setup': 3,
      'conversion_equation_setup': 4, 'data_control': 5, 'system_setup': 6,
      'request_system_status': 7, 'request_channel_status': 8,
      'request_channel_data': 9, 'digital_data_capture': 12, 'baudrate': 105,
      'request_channel_setup': 115, 'archive': 201, 'analog_output': 401,
      'led': 1998, 'sound': 1999, 'dig_out': 2001, 'power_control': 102}
  ...
  def __init__(self, port_name, highspeed=0):
    ...
    self.wake()
    self.wake()
    # try to wake up comunications by getting one good interchange
    for i in range(3):
      try:
        config = self.get_system_config()
      except:
        if i == 2:
          raise  # last try failed, allow exception out
      else:
  ...
  def setup_data_collection(
          self, samptime=0.1, numpoints=100, trigtype=0, trigchan=0,
    ...
    self.command(
        'data_collection_setup', samptime, numpoints, trigtype, trigchan,
        trigthresh, prestore, 0, rectime, filter, fastmode, delay=0.1)
  ...
  def wait_for_data_done(self, flasher=0):
    ...
    if misses:
      raise LabProError(
          "5 consecutives retry failures in wait_for_data_done()... bailing out")
  ...
  def parse_binary(self, data, channels, chunklen):
    ...
    if (chk != 0 and chk != 0xff):
      raise LabProDataError(
          "Bad checksum on string: " + ('%02x ' % chk) + repr(data))
    format = '>' + channels * 'H' + 'L'  # data+time+check
    l = [
        list(struct.unpack(format, data[i: i + chunklen - 1]))
  ...
  def scale_binary_data(self, idata, scaled_range):
    ...
    return Numeric.array(
        idata, numeric_float) * (
        (scaled_max - scaled_min) / 65536.0) + scaled_min

  def get_data_binary(
          self, chan=0, points=None, scaled_range=None, max_wait_time=1.0,
          loop_sleep_time=0.1):
    ...
    while(empties < maxloops and len(s) < chunklen):
      ...
      if not s and newdata:
        # remember when transmission started, in case someone cares
        self.data_transmission_start = time.time()
      s += newdata
      if newdata:
        empties = 0
      else:
    ...
    if empties:
      raise LabProTimeout(
          'timeout getting binary data, got %d bytes, expected %d, stuff looks like: %s'
          % (len(s),
             chunklen, repr(s[: 100])))
    ...
    if not (chk == 0 or chk == 0xff):
      raise LabProDataError(
          "Bad checksum on string: " + ('%02x ' % chk) + repr(s[:100]))
    data = array.array('H')
    data.fromstring(s[:-1])
    if not _bigendian:
      data.byteswap()
  ...
  def analog_output(
          self, waveform='dc', center=0.0, amplitude=1.0, period_sec=1.0):
    ...
    self.command(
        'analog_output', mode, ampl, offset, int(1000.0 * period_sec),
        delay=0.1)
  ...
  def sound(self, frequency=500.0, duration=0.1):
    "make a beep"
    self.command(
        'sound', int(0.5 + duration * 1e4),
        int(0.5 + 4e4 / frequency))
  ...
  def get_directory_labels(self, dir_type=1, calc_type=None):
    ...
    for i in range(count):
      if calc_type is None or dir_type != 2:
        ...
        # request name string for calculator-specific program
        self.command('archive', 3, dir_type, i + 1, calc_type)
  ...
  def create_new_archive_list(self, name=None, data=(), ident1=0, ident2=0):
    ...
    while 1:
      self.send_string('g')
      if self.read_ascii_response()[0] == 0:
        break  # wait for allocation & garbage collection
...
try:
  ...
  class termios_mixin:
    ...
    def set_port_params(self, baud=termios.B38400):
      ...
      cc = attrs[6]
      cc[termios.VMIN] = 0
      cc[termios.VTIME] = 0  # non-blocking reads
    ...
    def setup_serial(self, port_name):
      ...
      try:
        # see if seeking clears EOF (works on MacOSX, not on Linux)
        self.serial_read_port.seek(0)
    ...
    def high_speed_serial(self):
      ...
      # should work now, since it worked before
      self.set_port_params(baud=termios.B115200)
      time.sleep(0.1)
  ...
  # flag that this worked, don't look for other methods to make default
  _have_default_labpro = 1

except:  # no termios serial support
  # insert check for other system architectures here, such as Windows or
  # MacOS classic
  pass

if not _have_default_labpro:
  try:
    # this code written by Michael Mendenhall, Vanderbilt University, June
    # 2003, for use under Mac OS Classic python (not Carbon!)
    import ctb
...
try:
  ...
  class e5810_serial_mixin:
    ...
    def setup_serial(self, port_name=None):
      self.vxi11_host = vxi_11.vxi_11_connection(
          host=port_name, device="COM1", raise_on_err=1, timeout=5000,
    ...
    def read(self, maxlen=None):
      # last element of read is actual data
      return self.vxi11_host.read(count=maxlen)[-1]
  ...
  def try_labpro(binary=1):
    ...
    try:
      ...
      if 1:
        lp.reset()
        lp.analog_output(
            waveform='sine', center=-0.25, amplitude=4.0, period_sec=0.05)
        lp.setup_channel(chan=4)  # let channel read back its own data
        # current monitor (hidden command)
        lp.setup_channel(chan=2, operation=22)
        ...
        lp.wait_for_data_done()
        if binary:
          current = lp.get_data_binary(
              chan=2, points=npoints, scaled_range=(-0.1, 0.1))
          voltage = lp.get_data_binary(
              chan=4, points=npoints, scaled_range=(-10.0, 10.0))
        else:
