      ...
      if lock_fail_test:
        class slow_lock(vxi_crate_devices.e1413b):
          ...
          def __init__(self, slot=2):
            vxi_crate_devices.vxi_crate_device.init(
                self, slot, "slow-locking adc")
          ...
          try:
            ...
            for i in muxactive:
              ...
              muxdata += "%12s: %4d %6.1f %4d\n" % (
                  i, state, timer - time.time(),
                  prio)
          finally:
            muxhost[mux.host_lock].release()
          mainsystem.log_info(muxdata)

          # nest call in an extra lock, so we control the unlock
          pattern.lock()
          pattern.send(clock)  # stress test this
          pattern.unlock(-10)  # unlock and mark very stale

          scan_adc.log_info(
              "Loops " + str(scan_adc.loop_count) + " data: " +
              str(data["analog_sample"]))
          thermo.log_info(thermo.loop_count)
          glue_board.log_info(
              "%d %02x" % (glue_board.loop_count, data["glue"]))
          dio.log_info("%d %024x %024x" %
                       (dio.loop_count, data.dio96, dio.unstable))
