...
class handler(NATI_DSTP_stream_handler):
  ...
  def handle_connection(self):
    ...
    r = self.request
    # record this so server can close us, if needed
    self.server.request_closers[self] = self.close
...
class DSTPServer(SocketServer.ThreadingTCPServer, NATI_DSTP_DataServer):
  ...
  def close(self):
    ...
    # iterate over frozen copy of keys since idcionary may be changing
    requests = tuple(self.request_closers.keys())
