...
class ran_shift(random.Random):
  ...
  def seed(self, seed=None):
    ...
    seeds = [
        int(
            ((long(math.floor(random.random() * 32768.0)) << 16) &
             0x7fff0000) | long(math.floor(random.random() * 65536.0)))
        for i in range(self.p)]
    self.ranbuf = Numeric.array(seeds, numeric_int32)

    # scramble the dubious randoms from the default generator to start the
    # system
    for i in range(self.p):
      self.regenerate()
  ...
  def getstate(self):
    # make sure we copy the array, not just return a reference
    return Numeric.array(self.ranbuf), self.counter

  def setstate(self, state):
    # make a copy here, too, so we can re-use the state as often as needed
    self.ranbuf = Numeric.array(state[0])
  ...
  def fast_random_series(self, count):
    ...
    # how many are already in the buffer that we want?
    first = min(count, self.p - self.counter)
  ...
  def single_float_random_series(self, count):
    ...
    # take bits from the random pool.
    q1 = Numeric.array(self.fast_random_series(count), numeric_float)
  ...
  def double_float_random_series(self, count):
    ...
    # mask off low bits to prevent rounding when combined to 52-bit mantissas
    q1[1::2] &= self.mask32
    # convert results to doubles scaled on [0,1)
    q1 = q1 * self.single_floatscale
    # now, for any generator with two-point correlations, or unknown bit
    # lengths, this would fail horribly, but this class is safe.
...
class r250_521(r250):
  ...
  def fast_random_series(self, count):
    return r250.fast_random_series(
        self, count) ^ self.r521.fast_random_series(count)
